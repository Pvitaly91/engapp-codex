...
// Public site search endpoint (JSON for autocomplete, HTML for results page)
Route::get('/search', SiteSearchController::class)->name('site.search');

// Public word dictionary endpoint (JSON)
...
// Public test view routes (no authentication required) - Test V2 UI
...
// Redirects from old admin URLs to new public URLs
...
Route::middleware('auth.admin')->group(function () use ($reservedPrefixes) {
    ...
    // Old catalog routes (kept for backwards compatibility in admin)
    ...
    // Old test routes with /js suffix (admin/legacy access)
