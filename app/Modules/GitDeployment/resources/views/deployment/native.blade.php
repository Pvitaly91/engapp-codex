...
@section('content')
  <div class="max-w-4xl mx-auto space-y-8">
    ...
    @include('git-deployment::deployment.partials.content-sync-repair-card', [
      ...
      'syncPreviewRoute' => route('deployment.native.content-sync-preview'),
      'syncApplyRoute' => route('deployment.native.content-sync'),
    ])
            ...
            <label class="flex items-start gap-3 text-sm">
              ...
              <span>
                ...
                <span class="mt-1 block text-xs text-muted-foreground">
                  Після успішного API deploy буде запущено unified changed-content apply з тими самими base/head refs.
                </span>
              </span>
            </label>
    ...
    <section class="rounded-3xl border border-border/70 bg-card shadow-soft">
      <div class="space-y-6 p-6">
        <div>
          ...
          <p class="text-sm text-muted-foreground">Створює повний SQL-дамп MySQL бази у tracked-папці репозиторію. За потреби формує GitHub API commit тільки для файлу дампу без SSH-команд.</p>
        ...
        <form method="POST" action="{{ route('deployment.native.database-dump') }}" class="space-y-4">
          @csrf
          <div class="grid gap-4 md:grid-cols-2">
            <div class="space-y-2">
              <label class="block text-sm font-medium" for="native-database-dump-path">Файл дампу</label>
              <input
                id="native-database-dump-path"
                type="text"
                name="dump_path"
            ...
            <div class="space-y-2">
              <label class="block text-sm font-medium" for="native-database-dump-rows-per-insert">Рядків в одному INSERT</label>
              <input
                id="native-database-dump-rows-per-insert"
                type="number"
          ...
          <div class="rounded-2xl border border-border/70 bg-muted/30 p-4">
            <label class="flex items-start gap-3 text-sm">
              ...
              <span>
                ...
                <span class="mt-1 block text-xs text-muted-foreground">
                  Буде створено GitHub API commit тільки для файлу дампу і оновлено вказану гілку.
                </span>
              </span>
            </label>
            <div class="mt-3 space-y-2">
              <label class="block text-xs font-medium text-muted-foreground" for="native-database-dump-push-branch">Гілка для push</label>
              <input
                id="native-database-dump-push-branch"
                type="text"
                name="push_branch"
                value="{{ $feedback['branch'] ?? $currentBranch ?? 'main' }}"
            ...
            <div class="rounded-2xl border border-border/70 bg-muted/30 p-4">
              ...
              <label class="flex items-start gap-3 text-sm">
                ...
                <span>
                  ...
                  <span class="mt-1 block text-xs text-muted-foreground">
                    Після успішного API restore буде запущено unified changed-content apply з restore diff refs.
                  </span>
              </span>
            </label>
            <label class="mt-3 flex items-start gap-3 text-xs text-muted-foreground">
