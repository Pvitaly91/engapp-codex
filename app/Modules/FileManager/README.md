...
- Підсвітка синтаксису для популярних мов (PHP, Blade/HTML, CSS, JS тощо) з автоматичним завантаженням підсвітки навіть при відкритті сторінки за прямим посиланням та з урахуванням CSP (ресурси вантажаться з вашого домену і кешуються локально)
- Вбудований редактор текстових файлів (для файлів з правами на запис)
- Автоматичний перехід на простий текстовий редактор, якщо CodeMirror недоступний (CDN недосяжний тощо)
- Завантаження файлів
...
- Вбудований редактор відкриває файл із підсвіткою синтаксису для PHP, Blade/HTML, CSS, JavaScript, Markdown, SQL, shell-скриптів тощо (вміст файлу завантажується без додаткових обмежень на розмір)
...
- `GET /admin/file-manager/open/{path?}` - Пряме відкриття конкретної папки або файлу одним URL
...
- `GET /admin/file-manager/content?path={path}` - Завантажити повний вміст текстового файлу для редагування (без обмеження на розмір, лише для текстових файлів)
- `GET /admin/file-manager/assets/{path}` - Отримати ресурс CodeMirror/Highlight.js/Alpine (кешується у `storage/app/file-manager-assets`)
...
Файловий менеджер підтримує відкриття конкретної директорії або файлу як через прямий маршрут, так і через query-параметри:

- прямий маршрут `open/{path}`. Якщо передати шлях до папки, менеджер одразу відкриє її. Якщо передати шлях до файлу, менеджер відкриє батьківську директорію і виділить потрібний файл.
- приклад прямого лінку: `{{ route('file-manager.show', ['path' => 'database/migrations/2023_01_01_000000_create_users_table.php']) }}`
