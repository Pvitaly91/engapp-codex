...
class SiteTreeController extends Controller
{
    ...
    public function linkToPages(Request $request): JsonResponse
    {
        $variantId = $request->input('variant_id');
        ...
        try {
            ...
            return response()->json([
                'success' => true,
                'message' => "Створено {$linkedCount} нових зв'язків, оновлено {$updatedCount} існуючих",
                'linked' => $linkedCount,
                'updated' => $updatedCount,
            ]);
        } catch (\Exception $e) {
            Log::error('Failed to link site tree items to pages', [
                'error' => $e->getMessage(),
    ...
    private function getPageUrlMap(): array
    {
        $map = [];
    ...
    private function findPageMatch(string $title, array $pagesWithUrls): ?array
    {
