...
class TestTagController extends Controller
{
    ...
    {
        return view('test-tags.categories.create');
    }
    ...
    public function storeCategory(Request $request): RedirectResponse
    {
        ...
        $duplicateExists = TagCategory::query()
            ...
            ->exists()
            || Tag::query()
                ->whereRaw('LOWER(category) = ?', [$lowerName])
                ->exists();
        ...
        if ($duplicateExists) {
            return redirect()
                ->back()
                ->withInput()
                ->withErrors(['name' => 'Категорія з такою назвою вже існує.']);
        }

        TagCategory::create(['name' => $name]);

        return redirect()->route('test-tags.index')->with('status', 'Категорію створено.');
    }
