...
class PageController extends Controller
{
    ...
    public function show(PageCategory $category, string $pageSlug)
    {
        ...
        // Get persisted tests linked by the V3 prompt generator and merge them with virtual topic tests.
        $promptLinkedTests = $this->theoryPagePromptLinkedTestsService->findForPage($page);
        $autoGeneratedTests = $this->mergeTopicTests(
            $promptLinkedTests,
            $this->autoGeneratedTestService->generateTests($page->tags, $page->title)
        );
    ...
    protected function mergeTopicTests(Collection $primary, Collection $secondary): Collection
    {
        return $primary
            ->concat($secondary)
            ->unique(fn ($test) => $test->slug ?? spl_object_hash($test))
            ->values();
    }
}
