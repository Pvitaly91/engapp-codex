...
class PageController extends Controller
{
    ...
    public function category(PageCategory $category)
    {
        ...
        $autoGeneratedTests = $this->usesTheoryTopicTests()
            ? $this->theoryPagePromptLinkedTestsService->buildForCategory(
                $category,
                $this->categoryPagesForAggregatedTopicTests($category)
            )
            : $this->autoGeneratedTestService->generateTests($category->tags, $category->title);
    ...
    public function show(PageCategory $category, string $pageSlug)
    {
        ...
        // Practice matching is expensive; only run it for blocks that actually render questions.
        $practiceBlocks = new \Illuminate\Database\Eloquent\Collection(
            $blocks->where('type', 'practice-set')->values()->all()
        );
        $practiceQuestionsByBlock = $practiceBlocks->isEmpty()
            ? []
            : $this->textBlockToQuestionsMatcherService->findQuestionsForTextBlocks($practiceBlocks, 5);
    ...
    protected function categoryPagesForAggregatedTopicTests(PageCategory $category): Collection
    {
        $categoryIds = $this->recursiveCategoryIds($category);
    ...
    protected function recursiveCategoryIds(PageCategory $category): array
    {
        ...
        while ($frontier !== []) {
            $children = PageCategory::query()
                ->whereIn('parent_id', $frontier)
                ->when($this->pageType === 'theory', fn ($query) => $query->where('type', 'theory'))
                ->when($this->resolvedCategoryLanguage, fn ($query, $language) => $query->where('language', $language))
                ->pluck('id')
    ...
    protected function buildCategoryList(?string $language): Collection
    {
        ...
        $this->siteTreeOrdering = $this->siteTreeOrdering ?? Cache::remember(
            'page-controller:site-tree-ordering:'.md5(json_encode($availableTitles, JSON_UNESCAPED_UNICODE) ?: ''),
            now()->addHour(),
            fn () => $this->siteTreeLinkOrdering($availableTitles)
        );
        $ordering = $this->siteTreeOrdering;
    ...
    protected function siteTreeLinkOrdering(array $availableTitles): array
    {
        ...
        $itemsByParent = SiteTreeItem::query()
            ->where('variant_id', $variant->id)
            ->orderBy('sort_order')
            ->get()
            ->groupBy(fn (SiteTreeItem $item) => $item->parent_id ? (int) $item->parent_id : 0);

        $ordering = [];
        $position = 0;

        $collectOrdering = function ($items) use (&$collectOrdering, &$ordering, &$position, $availableLookup, $itemsByParent) {
            foreach ($items as $item) {
                ...
                $children = $itemsByParent->get((int) $item->getKey(), collect());

                if ($children->isNotEmpty()) {
                    $collectOrdering($children);
                }
            }
        };

        $collectOrdering($itemsByParent->get(0, collect()));
