...
class PageController extends Controller
{
    ...
    public function __construct(
        ...
        private TextBlockToQuestionsMatcherService $textBlockMatcherService
    ) {}
    ...
    public function show(PageCategory $category, string $pageSlug)
    {
        ...
        // Get practice questions for each text block
        $textBlockPracticeQuestions = $this->getTextBlockPracticeQuestions($blocks);
        ...
        return view($this->showView, [
            ...
            'relatedTests' => $relatedTests,
            'autoGeneratedTests' => $autoGeneratedTests,
            'textBlockPracticeQuestions' => $textBlockPracticeQuestions,
            'routePrefix' => $this->routePrefix,
            'sectionTitle' => $this->sectionTitle,
        ]);
    }
    ...
    /**
     * Get practice questions for each text block based on tag matching.
     *
     * This method uses the TextBlockToQuestionsMatcherService to find relevant
     * questions for each block. Questions must match ALL anchor tags (theme/type)
     * AND at least ONE detail tag (grammar feature) to be included.
     *
     * @param  Collection  $blocks  Collection of TextBlock models
     * @return array<int, Collection>  Keyed by block ID, value is collection of Question models
     */
    protected function getTextBlockPracticeQuestions(Collection $blocks): array
    {
        $result = [];
        $excludeQuestionIds = [];

        foreach ($blocks as $block) {
            // Only get practice questions for certain block types
            // Skip hero, navigation-chips, and similar structural blocks
            $skipTypes = ['hero', 'hero-v2', 'navigation-chips', 'subtitle'];
            if (in_array($block->type, $skipTypes, true)) {
                continue;
            }

            // Get matching questions for this block
            $questions = $this->textBlockMatcherService->findQuestionsForTextBlock(
                $block,
                limit: 3,
            ...
            if ($questions->isNotEmpty()) {
                $result[$block->id] = $questions;

                // Add to exclude list to avoid showing same questions under different blocks
                $excludeQuestionIds = array_merge(
                    $excludeQuestionIds,
