...
class PageController extends Controller
{
    ...
    public function show(PageCategory $category, string $pageSlug)
    {
        ...
        $practiceQuestionsByBlock = [];
        $usedQuestionIds = [];
        $practiceQuestionCountsByBlock = [];

        if ($this->pageType === 'theory') {
            $this->textBlockQuestionMatcherService->primeCandidatesForBlocks($blocks);
            ...
            foreach ($blocks as $block) {
                ...
                $practiceQuestionCountsByBlock[$block->uuid] = $limit;
                $questions = $this->textBlockQuestionMatcherService
                    ->findQuestionsForTextBlockWithExclusions($block, $limit, $usedQuestionIds);

                if ($questions->count() < $limit) {
                    $questions = $this->textBlockQuestionMatcherService
                        ->findQuestionsForTextBlockWithExclusions($block, $limit, []);
                }

                if ($questions->isNotEmpty()) {
                    $practiceQuestionsByBlock[$block->uuid] = $questions;
                    $usedQuestionIds = array_values(array_unique(array_merge(
                        $usedQuestionIds,
