...
class GrammarTestController extends Controller
{
    ...
    /**
     * Generate and display an auto-generated test based on filters
     */
    public function showAutoGeneratedTest(Request $request)
    {
        $request->validate([
            'level_from' => 'required|string',
            'level_to' => 'required|string',
            'tags' => 'required|array',
            'tags.*' => 'string',
        ]);
        ...
        $data = $this->filterService->prepare($filters);
        $questions = $data['questions'];

        $testName = "Авто-тест {$levelFrom}-{$levelTo}";
        if (!empty($tags)) {
            $testName .= ' (' . implode(', ', array_slice($tags, 0, 3)) . ')';
        }
        ...
        return view('auto-generated-test', [
            'testName' => $testName,
            'questions' => $questions,
            'levelFrom' => $levelFrom,
            'levelTo' => $levelTo,
