...
class V3DeletedPackageCleanupService extends V3PackageUnseedService
{
    /**
    ...
    public function runPackageRecord(array $packageRecord, array $options = []): array
    {
        ...
        try {
            ...
            $savedTestMetadata = [
                'uuid' => (string) data_get($packageRecord, 'historical_definition_summary.saved_test_uuid', ''),
                'slug' => (string) data_get($packageRecord, 'historical_definition_summary.saved_test_slug', ''),
            ];
            $context = $this->buildContext(
                $savedTestMetadata,
                $resolvedSeederClass,
                $this->packageQuestionUuidsForSeeder($resolvedSeederClass)
            );
            ...
            /** @var SavedGrammarTest|null $canonicalSavedTest */
            $canonicalSavedTest = $context['canonical_saved_test'];
