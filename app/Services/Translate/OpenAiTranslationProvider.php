...
class OpenAiTranslationProvider implements TranslationProviderInterface
{
    ...
    public function __construct()
    {
        ...
        // Increased default timeout for large content (JSON with HTML can be complex)
            ...
            $response = Http::timeout($dynamicTimeout)
                ...
                ->post('https://api.openai.com/v1/chat/completions', [
                    ...
                    'messages' => [
                        [
                            'role' => 'system',
                            'content' => $this->getSystemPrompt($contentAnalysis)
                        ],
                        [
                            'role' => 'user',
                            'content' => $prompt
    ...
    /**
     * Get system prompt based on content analysis
     */
    private function getSystemPrompt(array $contentAnalysis): string
    {
        $prompt = 'You are a professional translator specializing in UI/interface translations. Always respond with valid JSON only. Never add explanations or comments.';

        if ($contentAnalysis['hasJson']) {
            $prompt .= ' You are expert at translating text values within JSON structures while preserving the JSON structure perfectly.';
        }

        if ($contentAnalysis['hasHtml']) {
            $prompt .= ' You preserve all HTML tags exactly as they are, translating only the text content between tags.';
        }
