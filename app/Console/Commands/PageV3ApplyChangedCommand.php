...
class PageV3ApplyChangedCommand extends Command
{
    protected $signature = 'page-v3:apply-changed
        ...
        {--write-report : Write a Markdown report into storage/app/changed-package-apply-reports/page-v3}
    ...
    protected $description = 'Apply git-diff-aware Page_V3 changed packages by cleaning deleted packages first, then seeding or refreshing current ones.';

    public function __construct(
        private readonly PageV3ChangedPackagesApplyService $changedPackagesApplyService,
    ) {
        parent::__construct();
    }
    ...
    private function renderHumanOutput(array $result): void
    {
        ...
        $this->line('Command: page-v3:apply-changed');
        $this->line('Diff Mode: ' . (string) ($result['diff']['mode'] ?? 'working_tree'));
        $this->line('Target Scope: ' . (string) (($result['scope']['input'] ?? null) ?: 'database/seeders/Page_V3'));
