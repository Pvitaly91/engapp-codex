...
class V3PlanChangedCommand extends Command
{
    protected $signature = 'v3:plan-changed
        ...
        {--write-report : Write a Markdown report into storage/app/changed-package-plans/v3}
        {--strict : Treat planner warnings, inconsistent states, and release-check warnings as failures}';

    protected $description = 'Build a read-only git-diff-aware changed-package plan for V3 JSON packages.';
    ...
    private function renderHumanOutput(array $result): void
    {
        ...
        $this->line('Command: v3:plan-changed');
        $this->line('Diff Mode: ' . (string) ($result['diff']['mode'] ?? 'working_tree'));
        $this->line('Target Scope: ' . (string) ($result['scope']['input'] ?? 'database/seeders/V3'));
