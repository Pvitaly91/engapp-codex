<?php $__env->startSection('title', $test->name . ' - Step Test'); ?>

<?php $__env->startSection('content'); ?>
<div class="w-[800px] mx-auto p-4">
    <div class="flex items-center justify-between mb-4">
        <h1 class="text-2xl font-bold"><?php echo e($test->name); ?> - Step Mode</h1>
        <a href="<?php echo e(route('saved-test.show', $test->slug)); ?>" class="text-sm text-blue-600 underline">Back</a>
    </div>
    <?php if($test->description): ?>
        <div x-data="{ open: false }" class="test-description text-gray-800 flex justify-between">
            ...
            <div class="ml-2 space-x-2">
                <form method="POST" action="<?php echo e(route('saved-test.refresh', $test->slug)); ?>" class="inline">
                    <?php echo csrf_field(); ?>
                    <button type="submit" class="text-xs text-blue-600 underline">Оновити опис</button>
                </form>
                <form method="POST" action="<?php echo e(route('saved-test.refresh-gemini', $test->slug)); ?>" class="inline">
                    <?php echo csrf_field(); ?>
    ...
    <div class="mb-4 text-sm">
        ...
        <a href="<?php echo e(route('saved-test.step', ['slug' => $test->slug, 'order' => 'sequential'])); ?>" class="underline mr-2 <?php echo e($order === 'sequential' ? 'font-bold' : ''); ?>">По порядку</a>
        <a href="<?php echo e(route('saved-test.step', ['slug' => $test->slug, 'order' => 'random'])); ?>" class="underline <?php echo e($order === 'random' ? 'font-bold' : ''); ?>">Випадково</a>
    </div>
    <div class="mb-4 flex gap-4 text-gray-600 text-base">
        <div>Question: <b><?php echo e($questionNumber); ?> / <?php echo e($totalCount); ?></b></div>
        <div>Total: <b><?php echo e($stats['total']); ?> / <?php echo e($totalCount); ?></b></div>
        <div>Correct: <b class="text-green-700"><?php echo e($stats['correct']); ?></b></div>
        <div>Wrong: <b class="text-red-700"><?php echo e($stats['wrong']); ?></b></div>
        <div>Percent: <b><?php echo e($percentage); ?>%</b></div>
    </div>
    <div class="mb-4 flex gap-2">
        <form method="POST" action="<?php echo e(route('saved-test.step.reset', $test->slug)); ?>">
            <?php echo csrf_field(); ?>
    ...
    <div class="mb-4 flex justify-between">
        <?php if($hasPrev): ?>
            <a href="<?php echo e(route('saved-test.step', ['slug' => $test->slug, 'nav' => 'prev', 'order' => $order])); ?>" class="bg-gray-200 px-4 py-1 rounded hover:bg-gray-300 transition text-sm">Prev</a>
        <?php else: ?>
            <span class="bg-gray-100 px-4 py-1 rounded text-sm text-gray-400">Prev</span>
        <?php endif; ?>
        <?php if($hasNext): ?>
            <a href="<?php echo e(route('saved-test.step', ['slug' => $test->slug, 'nav' => 'next', 'order' => $order])); ?>" class="bg-gray-200 px-4 py-1 rounded hover:bg-gray-300 transition text-sm">Next</a>
        <?php else: ?>
    ...
    <?php if(isset($feedback)): ?>
        <div class="mb-4">
            ...
            <?php else: ?>
                ...
                <?php if(!empty($feedback['answer_sentence'])): ?>
                    <div class="text-sm text-gray-800 mb-2">Your answer: <?php echo $feedback['answer_sentence']; ?></div>
                <?php endif; ?>
                <?php if(!empty($feedback['explanations'])): ?>
                    <div class="bg-blue-50 text-gray-800 text-sm rounded px-3 py-2 space-y-1">
                        <?php $__currentLoopData = $feedback['explanations']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $exp): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <p><?php echo $exp; ?></p>
    ...
    <form method="POST" action="<?php echo e(route('saved-test.step.check', $test->slug)); ?>" class="space-y-4">
        <?php echo csrf_field(); ?>
        ...
        <div class="flex gap-2 mt-2">
            <a href="<?php echo e(route('question-review.edit', $question->id)); ?>" class="text-sm text-blue-600 underline">Review</a>
            <button type="submit" form="delete-question-<?php echo e($question->id); ?>" class="text-sm text-red-600 underline" onclick="return confirm('Delete this question?')">Delete</button>
        </div>
        <?php
    ...
    <form id="delete-question-<?php echo e($question->id); ?>" action="<?php echo e(route('saved-test.question.destroy', [$test->slug, $question->id])); ?>" method="POST" class="hidden">
        <?php echo csrf_field(); ?>
...
const tagColors = <?php echo json_encode($colors, 15, 512) ?>;
let selectedGptTags = [];
...
function renderTags(tags) {
    ...
    tags.forEach((name, index) => {
        ...
        link.href = '<?php echo e(route('saved-tests.cards')); ?>?tag=' + encodeURIComponent(name);
...
function addTag(tag) {
    return fetch('<?php echo e(route('saved-test.step.add-tag', $test->slug)); ?>', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
        },
        body: JSON.stringify({question_id: <?php echo e($question->id); ?>, tag})
    })
        .then(r => r.json())
        .then(d => {
...
function removeTag(tag) {
    return fetch('<?php echo e(route('saved-test.step.remove-tag', $test->slug)); ?>', {
        method: 'DELETE',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
...
renderTags(<?php echo json_encode($question->tags->pluck('name'), 15, 512) ?>);
...
function setLevel(level) {
    fetch('<?php echo e(route('saved-test.step.set-level', $test->slug)); ?>', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
        },
        body: JSON.stringify({question_id: <?php echo e($question->id); ?>, level})
    }).then(() => renderLevel(level));
}

renderLevel(<?php echo json_encode($question->level, 15, 512) ?>);

document.getElementById('determine-tense-gpt').addEventListener('click', () => {
    fetch('<?php echo e(route('saved-test.step.determine-tense', $test->slug)); ?>', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
        },
        body: JSON.stringify({question_id: <?php echo e($question->id); ?> })
    })
        .then(r => r.json())
        .then(d => {
...
document.getElementById('determine-tense-gemini').addEventListener('click', () => {
    fetch('<?php echo e(route('saved-test.step.determine-tense-gemini', $test->slug)); ?>', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
...
document.getElementById('determine-level-gpt').addEventListener('click', () => {
    fetch('<?php echo e(route('saved-test.step.determine-level', $test->slug)); ?>', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
...
document.getElementById('determine-level-gemini').addEventListener('click', () => {
    fetch('<?php echo e(route('saved-test.step.determine-level-gemini', $test->slug)); ?>', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH G:\DEV\htdocs\engapp-codex.loc\resources\views/saved-test-step.blade.php ENDPATH**/ ?>