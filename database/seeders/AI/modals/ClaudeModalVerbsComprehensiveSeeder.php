...
class ClaudeModalVerbsComprehensiveSeeder extends QuestionSeeder
{
    ...
    {
        $categoryId = Category::firstOrCreate(['name' => 'Claude Modal Verbs Comprehensive Practice'])->id;

        $sources = [];
        foreach ($this->subthemes as $key => $config) {
            $label = $config['label'];
            $sources[$key] = [
                'bank' => Source::firstOrCreate(['name' => "Claude: {$label} — Bank"])->id,
                'leveled' => Source::firstOrCreate(['name' => "Claude: {$label} — Leveled"])->id,
            ];
        }
