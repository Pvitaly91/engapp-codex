...
class PolyglotV3SeedersTest extends TestCase
{
    ...
    public function test_generator_driven_a2_lesson_one_seeder_works_and_stays_idempotent(): void
    {
        ...
        $lesson = SavedGrammarTest::query()
            ->with('questionLinks')
            ->where('slug', 'polyglot-present-perfect-basic-a2')
            ->firstOrFail();
        $questionUuids = collect(range(1, 24))
            ->map(fn (int $number) => sprintf('polyglot-present-perfect-basic-a2-q%02d', $number))
            ->all();
        ...
        $this->assertSame('polyglot-present-perfect-vs-past-simple-a2', $lesson->filters['next_lesson_slug'] ?? null);
        $this->assertTrue((bool) ($lesson->filters['supports_duplicate_tokens'] ?? false));
        $this->assertSame(
            'Database\\Seeders\\Page_V3\\Tenses\\PresentPerfect\\PresentPerfectFormsTheorySeeder',
            $theoryPage['page_seeder_class'] ?? null
        );
        $this->assertSame('present-perfect-forms', $theoryPage['slug'] ?? null);
        $this->assertSame('tenses/present-perfect', $theoryPage['category_slug_path'] ?? null);
        $this->assertSame(
            1,
            SavedGrammarTest::query()->where('slug', 'polyglot-present-perfect-basic-a2')->count()
        );
    }

    public function test_generator_driven_a2_lesson_one_package_exists_in_canonical_v3_structure(): void
    {
        $loaderPath = database_path('seeders/V3/Polyglot/PolyglotPresentPerfectBasicLessonSeeder.php');
        $packagePath = database_path('seeders/V3/Polyglot/PolyglotPresentPerfectBasicLessonSeeder');
        $definitionPath = $packagePath . '/definition.json';
        $realSeederPath = $packagePath . '/PolyglotPresentPerfectBasicLessonSeeder.php';
        $ukPath = $packagePath . '/localizations/uk.json';
        $enPath = $packagePath . '/localizations/en.json';
        $plPath = $packagePath . '/localizations/pl.json';
        $promptPath = storage_path('app/polyglot-prompts/polyglot-present-perfect-basic-a2.txt');
        ...
        $this->assertFileExists($promptPath);
        $this->assertStringContainsString(
            "require_once __DIR__ . '/PolyglotPresentPerfectBasicLessonSeeder/PolyglotPresentPerfectBasicLessonSeeder.php';",
            File::get($loaderPath)
        );
        ...
        $this->assertSame(
            'Database\\Seeders\\V3\\Polyglot\\PolyglotPresentPerfectBasicLessonSeeder',
            $definition['seeder']['class'] ?? null
        );
        $this->assertSame('polyglot-present-perfect-basic-a2', $definition['saved_test']['slug'] ?? null);
        $this->assertSame(24, count($definition['questions'] ?? []));
        $this->assertSame(
            'Database\\Seeders\\V3\\Localizations\\Uk\\Polyglot\\PolyglotPresentPerfectBasicLessonLocalizationSeeder',
            $ukLocalization['seeder']['class'] ?? null
        );
        $this->assertStringContainsString('present-perfect-forms', File::get($promptPath));
        $this->assertStringContainsString('polyglot-english-a2', File::get($promptPath));
    }

    public function test_generator_driven_a2_lesson_two_seeder_works_and_stays_idempotent(): void
    {
        $this->seed(PolyglotPresentPerfectBasicLessonSeeder::class);
        ...
        $lesson = SavedGrammarTest::query()
            ->with('questionLinks')
            ->where('slug', 'polyglot-present-perfect-vs-past-simple-a2')
            ->firstOrFail();
        $questionUuids = collect(range(1, 24))
            ->map(fn (int $number) => sprintf('polyglot-present-perfect-vs-past-simple-a2-q%02d', $number))
            ->all();
        $persistentQuestionUuids = $resolver->toPersistentMany($questionUuids);
        $theoryPage = $lesson->filters['prompt_generator']['theory_page'] ?? [];
        ...
        $this->assertSame('polyglot-present-perfect-basic-a2', $lesson->filters['previous_lesson_slug'] ?? null);
        $this->assertSame('polyglot-first-conditional-a2', $lesson->filters['next_lesson_slug'] ?? null);
        $this->assertTrue((bool) ($lesson->filters['supports_duplicate_tokens'] ?? false));
        $this->assertSame(
            'Database\\Seeders\\Page_V3\\Tenses\\TensesPresentPerfectVsPastSimpleTheorySeeder',
            $theoryPage['page_seeder_class'] ?? null
        );
        $this->assertSame('present-perfect-vs-past-simple', $theoryPage['slug'] ?? null);
        $this->assertSame('tenses', $theoryPage['category_slug_path'] ?? null);
        $this->assertSame(
            1,
            SavedGrammarTest::query()->where('slug', 'polyglot-present-perfect-vs-past-simple-a2')->count()
        );
    }

    public function test_generator_driven_a2_lesson_two_package_exists_in_canonical_v3_structure(): void
    {
        $loaderPath = database_path('seeders/V3/Polyglot/PolyglotPresentPerfectVsPastSimpleLessonSeeder.php');
        $packagePath = database_path('seeders/V3/Polyglot/PolyglotPresentPerfectVsPastSimpleLessonSeeder');
        $definitionPath = $packagePath . '/definition.json';
        $realSeederPath = $packagePath . '/PolyglotPresentPerfectVsPastSimpleLessonSeeder.php';
        $ukPath = $packagePath . '/localizations/uk.json';
        $enPath = $packagePath . '/localizations/en.json';
        $plPath = $packagePath . '/localizations/pl.json';
        $promptPath = storage_path('app/polyglot-prompts/polyglot-present-perfect-vs-past-simple-a2.txt');
        ...
        $this->assertFileExists($promptPath);
        $this->assertStringContainsString(
            "require_once __DIR__ . '/PolyglotPresentPerfectVsPastSimpleLessonSeeder/PolyglotPresentPerfectVsPastSimpleLessonSeeder.php';",
            File::get($loaderPath)
        );
        ...
        $this->assertSame('polyglot-present-perfect-vs-past-simple-a2', $definition['saved_test']['slug'] ?? null);
        $this->assertSame(24, count($definition['questions'] ?? []));
        $this->assertSame(
            'polyglot-present-perfect-vs-past-simple-a2-q24',
            $definition['saved_test']['question_uuids'][23] ?? null
        );
        $this->assertSame(
            'Database\\Seeders\\V3\\Localizations\\Uk\\Polyglot\\PolyglotPresentPerfectVsPastSimpleLessonLocalizationSeeder',
            $ukLocalization['seeder']['class'] ?? null
        );
        $this->assertStringContainsString('present-perfect-vs-past-simple', File::get($promptPath));
        $this->assertStringContainsString('polyglot-english-a2', File::get($promptPath));
    }

    public function test_generator_driven_a2_lesson_three_seeder_works_and_stays_idempotent(): void
    {
        $this->seed(PolyglotPresentPerfectBasicLessonSeeder::class);
        $this->seed(PolyglotPresentPerfectVsPastSimpleLessonSeeder::class);
        ...
        $lesson = SavedGrammarTest::query()
            ->with('questionLinks')
            ->where('slug', 'polyglot-first-conditional-a2')
            ->firstOrFail();
        $questionUuids = collect(range(1, 24))
            ->map(fn (int $number) => sprintf('polyglot-first-conditional-a2-q%02d', $number))
            ->all();
        $persistentQuestionUuids = $resolver->toPersistentMany($questionUuids);
        $theoryPage = $lesson->filters['prompt_generator']['theory_page'] ?? [];
        ...
        $this->assertSame('polyglot-present-perfect-vs-past-simple-a2', $lesson->filters['previous_lesson_slug'] ?? null);
        $this->assertSame('polyglot-be-going-to-a2', $lesson->filters['next_lesson_slug'] ?? null);
        $this->assertTrue((bool) ($lesson->filters['supports_duplicate_tokens'] ?? false));
        $this->assertSame(
            'Database\\Seeders\\Page_V3\\Conditionals\\ConditionalsFirstTheorySeeder',
            $theoryPage['page_seeder_class'] ?? null
        );
        $this->assertSame('first-conditional', $theoryPage['slug'] ?? null);
        $this->assertSame('conditionals', $theoryPage['category_slug_path'] ?? null);
        $this->assertSame(
            1,
            SavedGrammarTest::query()->where('slug', 'polyglot-first-conditional-a2')->count()
        );
    }

    public function test_generator_driven_a2_lesson_three_package_exists_in_canonical_v3_structure(): void
    {
        $loaderPath = database_path('seeders/V3/Polyglot/PolyglotFirstConditionalLessonSeeder.php');
        $packagePath = database_path('seeders/V3/Polyglot/PolyglotFirstConditionalLessonSeeder');
        $definitionPath = $packagePath . '/definition.json';
        $realSeederPath = $packagePath . '/PolyglotFirstConditionalLessonSeeder.php';
        $ukPath = $packagePath . '/localizations/uk.json';
        $enPath = $packagePath . '/localizations/en.json';
        $plPath = $packagePath . '/localizations/pl.json';
        $promptPath = storage_path('app/polyglot-prompts/polyglot-first-conditional-a2.txt');
        ...
        $this->assertFileExists($promptPath);
        $this->assertStringContainsString(
            "require_once __DIR__ . '/PolyglotFirstConditionalLessonSeeder/PolyglotFirstConditionalLessonSeeder.php';",
            File::get($loaderPath)
        );
        ...
        $this->assertSame(
            'Database\\Seeders\\V3\\Polyglot\\PolyglotFirstConditionalLessonSeeder',
            $definition['seeder']['class'] ?? null
        );
        $this->assertSame('polyglot-first-conditional-a2', $definition['saved_test']['slug'] ?? null);
        $this->assertSame(24, count($definition['questions'] ?? []));
        $this->assertSame(
            'polyglot-first-conditional-a2-q24',
            $definition['saved_test']['question_uuids'][23] ?? null
        );
        $this->assertSame(
            'Database\\Seeders\\V3\\Localizations\\Uk\\Polyglot\\PolyglotFirstConditionalLessonLocalizationSeeder',
            $ukLocalization['seeder']['class'] ?? null
        );
        $this->assertStringContainsString('first-conditional', File::get($promptPath));
        $this->assertStringContainsString('polyglot-english-a2', File::get($promptPath));
    }

    public function test_generator_driven_a2_lesson_four_seeder_works_and_stays_idempotent(): void
    {
        $this->seed(PolyglotPresentPerfectBasicLessonSeeder::class);
        $this->seed(PolyglotPresentPerfectVsPastSimpleLessonSeeder::class);
        $this->seed(PolyglotFirstConditionalLessonSeeder::class);
        ...
        $lesson = SavedGrammarTest::query()
            ->with('questionLinks')
            ->where('slug', 'polyglot-be-going-to-a2')
            ->firstOrFail();
        $questionUuids = collect(range(1, 24))
            ->map(fn (int $number) => sprintf('polyglot-be-going-to-a2-q%02d', $number))
            ->all();
        $persistentQuestionUuids = $resolver->toPersistentMany($questionUuids);
        $theoryPage = $lesson->filters['prompt_generator']['theory_page'] ?? [];
        ...
        $this->assertSame('polyglot-first-conditional-a2', $lesson->filters['previous_lesson_slug'] ?? null);
        $this->assertSame('polyglot-should-ought-to-a2', $lesson->filters['next_lesson_slug'] ?? null);
        $this->assertTrue((bool) ($lesson->filters['supports_duplicate_tokens'] ?? false));
        $this->assertSame(
            'Database\\Seeders\\Page_V3\\FutureForms\\FutureFormsWillVsBeGoingToTheorySeeder',
            $theoryPage['page_seeder_class'] ?? null
        );
        $this->assertSame('will-vs-be-going-to', $theoryPage['slug'] ?? null);
        $this->assertSame('maibutni-formy', $theoryPage['category_slug_path'] ?? null);
        $this->assertSame(
            1,
            SavedGrammarTest::query()->where('slug', 'polyglot-be-going-to-a2')->count()
        );
    }

    public function test_generator_driven_a2_lesson_four_package_exists_in_canonical_v3_structure(): void
    {
        $loaderPath = database_path('seeders/V3/Polyglot/PolyglotBeGoingToLessonSeeder.php');
        $packagePath = database_path('seeders/V3/Polyglot/PolyglotBeGoingToLessonSeeder');
        $definitionPath = $packagePath . '/definition.json';
        $realSeederPath = $packagePath . '/PolyglotBeGoingToLessonSeeder.php';
        $ukPath = $packagePath . '/localizations/uk.json';
        $enPath = $packagePath . '/localizations/en.json';
        $plPath = $packagePath . '/localizations/pl.json';
        $promptPath = storage_path('app/polyglot-prompts/polyglot-be-going-to-a2.txt');
        ...
        $this->assertFileExists($promptPath);
        $this->assertStringContainsString(
            "require_once __DIR__ . '/PolyglotBeGoingToLessonSeeder/PolyglotBeGoingToLessonSeeder.php';",
            File::get($loaderPath)
        );
        ...
        $this->assertSame(
            'Database\\Seeders\\V3\\Polyglot\\PolyglotBeGoingToLessonSeeder',
            $definition['seeder']['class'] ?? null
        );
        $this->assertSame('polyglot-be-going-to-a2', $definition['saved_test']['slug'] ?? null);
        $this->assertSame(24, count($definition['questions'] ?? []));
        $this->assertSame(
            'polyglot-be-going-to-a2-q24',
            $definition['saved_test']['question_uuids'][23] ?? null
        );
        $this->assertSame(
            'Database\\Seeders\\V3\\Localizations\\Uk\\Polyglot\\PolyglotBeGoingToLessonLocalizationSeeder',
            $ukLocalization['seeder']['class'] ?? null
        );
        $this->assertStringContainsString('will-vs-be-going-to', File::get($promptPath));
        $this->assertStringContainsString('polyglot-english-a2', File::get($promptPath));
    ...
    public function test_generator_driven_a2_lesson_five_seeder_works_and_stays_idempotent(): void
    {
        $this->seed(PolyglotPresentPerfectBasicLessonSeeder::class);
        $this->seed(PolyglotPresentPerfectVsPastSimpleLessonSeeder::class);
        $this->seed(PolyglotFirstConditionalLessonSeeder::class);
        ...
        $lesson = SavedGrammarTest::query()
            ->with('questionLinks')
            ->where('slug', 'polyglot-should-ought-to-a2')
            ->firstOrFail();
        $questionUuids = collect(range(1, 24))
            ->map(fn (int $number) => sprintf('polyglot-should-ought-to-a2-q%02d', $number))
            ->all();
        $persistentQuestionUuids = $resolver->toPersistentMany($questionUuids);
        $theoryPage = $lesson->filters['prompt_generator']['theory_page'] ?? [];
        ...
        $this->assertSame('polyglot-english-a2', $lesson->filters['course_slug'] ?? null);
        $this->assertSame(5, $lesson->filters['lesson_order'] ?? null);
        $this->assertSame('polyglot-be-going-to-a2', $lesson->filters['previous_lesson_slug'] ?? null);
        $this->assertSame('polyglot-must-have-to-a2', $lesson->filters['next_lesson_slug'] ?? null);
        $this->assertTrue((bool) ($lesson->filters['supports_duplicate_tokens'] ?? false));
        $this->assertSame(
            'Database\\Seeders\\Page_V3\\ModalVerbs\\ModalVerbsShouldOughtToTheorySeeder',
            $theoryPage['page_seeder_class'] ?? null
        );
        $this->assertSame('should-ought-to', $theoryPage['slug'] ?? null);
        $this->assertSame('modal-verbs', $theoryPage['category_slug_path'] ?? null);
        $this->assertSame(
            1,
            SavedGrammarTest::query()->where('slug', 'polyglot-should-ought-to-a2')->count()
        );
    }

    public function test_generator_driven_a2_lesson_five_package_exists_in_canonical_v3_structure(): void
    {
        $loaderPath = database_path('seeders/V3/Polyglot/PolyglotShouldOughtToLessonSeeder.php');
        $packagePath = database_path('seeders/V3/Polyglot/PolyglotShouldOughtToLessonSeeder');
        $definitionPath = $packagePath . '/definition.json';
        $realSeederPath = $packagePath . '/PolyglotShouldOughtToLessonSeeder.php';
        $ukPath = $packagePath . '/localizations/uk.json';
        $enPath = $packagePath . '/localizations/en.json';
        $plPath = $packagePath . '/localizations/pl.json';
        $promptPath = storage_path('app/polyglot-prompts/polyglot-should-ought-to-a2.txt');
        ...
        $this->assertFileExists($promptPath);
        $this->assertStringContainsString(
            "require_once __DIR__ . '/PolyglotShouldOughtToLessonSeeder/PolyglotShouldOughtToLessonSeeder.php';",
            File::get($loaderPath)
        );
        ...
        $this->assertSame(
            'Database\\Seeders\\V3\\Polyglot\\PolyglotShouldOughtToLessonSeeder',
            $definition['seeder']['class'] ?? null
        );
        $this->assertSame('polyglot-should-ought-to-a2', $definition['saved_test']['slug'] ?? null);
        $this->assertSame(24, count($definition['questions'] ?? []));
        $this->assertSame(
            'polyglot-should-ought-to-a2-q24',
            $definition['saved_test']['question_uuids'][23] ?? null
        );
        $this->assertSame(
            'Database\\Seeders\\V3\\Localizations\\Uk\\Polyglot\\PolyglotShouldOughtToLessonLocalizationSeeder',
            $ukLocalization['seeder']['class'] ?? null
        );
        $this->assertStringContainsString('should-ought-to', File::get($promptPath));
        $this->assertStringContainsString('polyglot-english-a2', File::get($promptPath));
    }

    public function test_generator_driven_a2_lesson_seven_seeder_works_and_stays_idempotent(): void
    {
        $this->seed(PolyglotPresentPerfectBasicLessonSeeder::class);
        $this->seed(PolyglotPresentPerfectVsPastSimpleLessonSeeder::class);
        $this->seed(PolyglotFirstConditionalLessonSeeder::class);
        ...
        $lesson = SavedGrammarTest::query()
            ->with('questionLinks')
            ->where('slug', 'polyglot-gerund-vs-infinitive-a2')
            ->firstOrFail();
        $questionUuids = collect(range(1, 24))
            ->map(fn (int $number) => sprintf('polyglot-gerund-vs-infinitive-a2-q%02d', $number))
            ->all();
        $persistentQuestionUuids = $resolver->toPersistentMany($questionUuids);
        $theoryPage = $lesson->filters['prompt_generator']['theory_page'] ?? [];
        ...
        $this->assertSame('polyglot-english-a2', $lesson->filters['course_slug'] ?? null);
        $this->assertSame(7, $lesson->filters['lesson_order'] ?? null);
        $this->assertSame('polyglot-must-have-to-a2', $lesson->filters['previous_lesson_slug'] ?? null);
        $this->assertSame('polyglot-past-continuous-a2', $lesson->filters['next_lesson_slug'] ?? null);
        $this->assertTrue((bool) ($lesson->filters['supports_duplicate_tokens'] ?? false));
        $this->assertSame(
            'Database\\Seeders\\Page_V3\\VerbPatterns\\VerbPatternsGerundVsInfinitiveTheorySeeder',
            $theoryPage['page_seeder_class'] ?? null
        );
        $this->assertSame('gerund-vs-infinitive', $theoryPage['slug'] ?? null);
        $this->assertSame('verb-patterns', $theoryPage['category_slug_path'] ?? null);
        $this->assertSame(
            1,
            SavedGrammarTest::query()->where('slug', 'polyglot-gerund-vs-infinitive-a2')->count()
        );
    }

    public function test_generator_driven_a2_lesson_seven_package_exists_in_canonical_v3_structure(): void
    {
        $loaderPath = database_path('seeders/V3/Polyglot/PolyglotGerundVsInfinitiveLessonSeeder.php');
        $packagePath = database_path('seeders/V3/Polyglot/PolyglotGerundVsInfinitiveLessonSeeder');
        $definitionPath = $packagePath . '/definition.json';
        $realSeederPath = $packagePath . '/PolyglotGerundVsInfinitiveLessonSeeder.php';
        $ukPath = $packagePath . '/localizations/uk.json';
        $enPath = $packagePath . '/localizations/en.json';
        $plPath = $packagePath . '/localizations/pl.json';
        $promptPath = storage_path('app/polyglot-prompts/polyglot-gerund-vs-infinitive-a2.txt');
        ...
        $this->assertFileExists($promptPath);
        $this->assertStringContainsString(
            "require_once __DIR__ . '/PolyglotGerundVsInfinitiveLessonSeeder/PolyglotGerundVsInfinitiveLessonSeeder.php';",
            File::get($loaderPath)
        );
        ...
        $this->assertSame(
            'Database\\Seeders\\V3\\Polyglot\\PolyglotGerundVsInfinitiveLessonSeeder',
            $definition['seeder']['class'] ?? null
        );
        $this->assertSame('polyglot-gerund-vs-infinitive-a2', $definition['saved_test']['slug'] ?? null);
        $this->assertSame(24, count($definition['questions'] ?? []));
        $this->assertSame(
            'polyglot-gerund-vs-infinitive-a2-q24',
            $definition['saved_test']['question_uuids'][23] ?? null
        );
        $this->assertSame(
            'Database\\Seeders\\V3\\Localizations\\Uk\\Polyglot\\PolyglotGerundVsInfinitiveLessonLocalizationSeeder',
            $ukLocalization['seeder']['class'] ?? null
        );
        $this->assertStringContainsString('gerund-vs-infinitive', File::get($promptPath));
        $this->assertStringContainsString('polyglot-english-a2', File::get($promptPath));
    }

    public function test_generator_driven_a2_lesson_eight_seeder_works_and_stays_idempotent(): void
    {
        $this->seed(PolyglotPresentPerfectBasicLessonSeeder::class);
        $this->seed(PolyglotPresentPerfectVsPastSimpleLessonSeeder::class);
        $this->seed(PolyglotFirstConditionalLessonSeeder::class);
        ...
        $lesson = SavedGrammarTest::query()
            ->with('questionLinks')
            ->where('slug', 'polyglot-past-continuous-a2')
            ->firstOrFail();
        $questionUuids = collect(range(1, 24))
            ->map(fn (int $number) => sprintf('polyglot-past-continuous-a2-q%02d', $number))
            ->all();
        $persistentQuestionUuids = $resolver->toPersistentMany($questionUuids);
        $theoryPage = $lesson->filters['prompt_generator']['theory_page'] ?? [];
        ...
        $this->assertSame('polyglot-english-a2', $lesson->filters['course_slug'] ?? null);
        $this->assertSame(8, $lesson->filters['lesson_order'] ?? null);
        $this->assertSame('polyglot-gerund-vs-infinitive-a2', $lesson->filters['previous_lesson_slug'] ?? null);
        $this->assertSame('polyglot-present-perfect-time-expressions-a2', $lesson->filters['next_lesson_slug'] ?? null);
        $this->assertTrue((bool) ($lesson->filters['supports_duplicate_tokens'] ?? false));
        $this->assertSame(
            'Database\\Seeders\\Page_V3\\Tenses\\PastContinuous\\PastContinuousFormsTheorySeeder',
            $theoryPage['page_seeder_class'] ?? null
        );
        $this->assertSame('past-continuous-forms', $theoryPage['slug'] ?? null);
        $this->assertSame('tenses/past-continuous', $theoryPage['category_slug_path'] ?? null);
        $this->assertSame(
            1,
            SavedGrammarTest::query()->where('slug', 'polyglot-past-continuous-a2')->count()
        );
    }

    public function test_generator_driven_a2_lesson_eight_package_exists_in_canonical_v3_structure(): void
    {
        $loaderPath = database_path('seeders/V3/Polyglot/PolyglotPastContinuousLessonSeeder.php');
        $packagePath = database_path('seeders/V3/Polyglot/PolyglotPastContinuousLessonSeeder');
        $definitionPath = $packagePath . '/definition.json';
        $realSeederPath = $packagePath . '/PolyglotPastContinuousLessonSeeder.php';
        $ukPath = $packagePath . '/localizations/uk.json';
        $enPath = $packagePath . '/localizations/en.json';
        $plPath = $packagePath . '/localizations/pl.json';
        $promptPath = storage_path('app/polyglot-prompts/polyglot-past-continuous-a2.txt');
        ...
        $this->assertFileExists($promptPath);
        $this->assertStringContainsString(
            "require_once __DIR__ . '/PolyglotPastContinuousLessonSeeder/PolyglotPastContinuousLessonSeeder.php';",
            File::get($loaderPath)
        );
        ...
        $this->assertSame(
            'Database\\Seeders\\V3\\Polyglot\\PolyglotPastContinuousLessonSeeder',
            $definition['seeder']['class'] ?? null
        );
        $this->assertSame('polyglot-past-continuous-a2', $definition['saved_test']['slug'] ?? null);
        $this->assertSame(24, count($definition['questions'] ?? []));
        $this->assertSame(
            'polyglot-past-continuous-a2-q24',
            $definition['saved_test']['question_uuids'][23] ?? null
        );
        $this->assertSame(
            'Database\\Seeders\\V3\\Localizations\\Uk\\Polyglot\\PolyglotPastContinuousLessonLocalizationSeeder',
            $ukLocalization['seeder']['class'] ?? null
        );
        $this->assertStringContainsString('past-continuous-forms', File::get($promptPath));
        $this->assertStringContainsString('polyglot-english-a2', File::get($promptPath));
    }

    public function test_generator_driven_a2_lesson_nine_seeder_works_and_stays_idempotent(): void
    {
        $this->seed(PolyglotPresentPerfectBasicLessonSeeder::class);
        $this->seed(PolyglotPresentPerfectVsPastSimpleLessonSeeder::class);
        $this->seed(PolyglotFirstConditionalLessonSeeder::class);
        ...
        $lesson = SavedGrammarTest::query()
            ->with('questionLinks')
            ->where('slug', 'polyglot-present-perfect-time-expressions-a2')
            ->firstOrFail();
        $questionUuids = collect(range(1, 24))
            ->map(fn (int $number) => sprintf('polyglot-present-perfect-time-expressions-a2-q%02d', $number))
            ->all();
        $persistentQuestionUuids = $resolver->toPersistentMany($questionUuids);
        $theoryPage = $lesson->filters['prompt_generator']['theory_page'] ?? [];
        ...
        $this->assertSame('polyglot-english-a2', $lesson->filters['course_slug'] ?? null);
        $this->assertSame(9, $lesson->filters['lesson_order'] ?? null);
        $this->assertSame('polyglot-past-continuous-a2', $lesson->filters['previous_lesson_slug'] ?? null);
        $this->assertSame('polyglot-relative-clauses-a2', $lesson->filters['next_lesson_slug'] ?? null);
        $this->assertTrue((bool) ($lesson->filters['supports_duplicate_tokens'] ?? false));
        $this->assertSame(
            'Database\\Seeders\\Page_V3\\Tenses\\PresentPerfect\\PresentPerfectTimeExpressionsTheorySeeder',
            $theoryPage['page_seeder_class'] ?? null
        );
        $this->assertSame('present-perfect-time-expressions', $theoryPage['slug'] ?? null);
        $this->assertSame('tenses/present-perfect', $theoryPage['category_slug_path'] ?? null);
        $this->assertSame(
            1,
            SavedGrammarTest::query()->where('slug', 'polyglot-present-perfect-time-expressions-a2')->count()
        );
    }

    public function test_generator_driven_a2_lesson_nine_package_exists_in_canonical_v3_structure(): void
    {
        $loaderPath = database_path('seeders/V3/Polyglot/PolyglotPresentPerfectTimeExpressionsLessonSeeder.php');
        $packagePath = database_path('seeders/V3/Polyglot/PolyglotPresentPerfectTimeExpressionsLessonSeeder');
        $definitionPath = $packagePath . '/definition.json';
        $realSeederPath = $packagePath . '/PolyglotPresentPerfectTimeExpressionsLessonSeeder.php';
        $ukPath = $packagePath . '/localizations/uk.json';
        $enPath = $packagePath . '/localizations/en.json';
        $plPath = $packagePath . '/localizations/pl.json';
        $promptPath = storage_path('app/polyglot-prompts/polyglot-present-perfect-time-expressions-a2.txt');
        ...
        $this->assertFileExists($promptPath);
        $this->assertStringContainsString(
            "require_once __DIR__ . '/PolyglotPresentPerfectTimeExpressionsLessonSeeder/PolyglotPresentPerfectTimeExpressionsLessonSeeder.php';",
            File::get($loaderPath)
        );
        ...
        $this->assertSame(
            'Database\\Seeders\\V3\\Polyglot\\PolyglotPresentPerfectTimeExpressionsLessonSeeder',
            $definition['seeder']['class'] ?? null
        );
        $this->assertSame('polyglot-present-perfect-time-expressions-a2', $definition['saved_test']['slug'] ?? null);
        $this->assertSame(24, count($definition['questions'] ?? []));
        $this->assertSame(
            'polyglot-present-perfect-time-expressions-a2-q24',
            $definition['saved_test']['question_uuids'][23] ?? null
        );
        $this->assertSame(
            'Database\\Seeders\\V3\\Localizations\\Uk\\Polyglot\\PolyglotPresentPerfectTimeExpressionsLessonLocalizationSeeder',
            $ukLocalization['seeder']['class'] ?? null
        );
        $this->assertStringContainsString('present-perfect-time-expressions', File::get($promptPath));
        $this->assertStringContainsString('polyglot-english-a2', File::get($promptPath));
    }

    public function test_generator_driven_a2_lesson_ten_seeder_works_and_stays_idempotent(): void
    {
        $this->seed(PolyglotPresentPerfectBasicLessonSeeder::class);
        $this->seed(PolyglotPresentPerfectVsPastSimpleLessonSeeder::class);
        $this->seed(PolyglotFirstConditionalLessonSeeder::class);
        ...
        $lesson = SavedGrammarTest::query()
            ->with('questionLinks')
            ->where('slug', 'polyglot-relative-clauses-a2')
            ->firstOrFail();
        $questionUuids = collect(range(1, 24))
            ->map(fn (int $number) => sprintf('polyglot-relative-clauses-a2-q%02d', $number))
            ->all();
        $persistentQuestionUuids = $resolver->toPersistentMany($questionUuids);
        $theoryPage = $lesson->filters['prompt_generator']['theory_page'] ?? [];
        ...
        $this->assertSame('polyglot-english-a2', $lesson->filters['course_slug'] ?? null);
        $this->assertSame(10, $lesson->filters['lesson_order'] ?? null);
        $this->assertSame('polyglot-present-perfect-time-expressions-a2', $lesson->filters['previous_lesson_slug'] ?? null);
        $this->assertSame('polyglot-passive-voice-basics-a2', $lesson->filters['next_lesson_slug'] ?? null);
        $this->assertTrue((bool) ($lesson->filters['supports_duplicate_tokens'] ?? false));
        $this->assertSame(
            'Database\\Seeders\\Page_V3\\ClausesAndLinkingWords\\ClausesAndLinkingWordsRelativeClausesTheorySeeder',
            $theoryPage['page_seeder_class'] ?? null
        );
        $this->assertSame('relative-clauses', $theoryPage['slug'] ?? null);
        $this->assertSame('clauses-and-linking-words', $theoryPage['category_slug_path'] ?? null);
        $this->assertSame(
            1,
            SavedGrammarTest::query()->where('slug', 'polyglot-relative-clauses-a2')->count()
        );
    }

    public function test_generator_driven_a2_lesson_ten_package_exists_in_canonical_v3_structure(): void
    {
        $loaderPath = database_path('seeders/V3/Polyglot/PolyglotRelativeClausesLessonSeeder.php');
        $packagePath = database_path('seeders/V3/Polyglot/PolyglotRelativeClausesLessonSeeder');
        $definitionPath = $packagePath . '/definition.json';
        $realSeederPath = $packagePath . '/PolyglotRelativeClausesLessonSeeder.php';
        $ukPath = $packagePath . '/localizations/uk.json';
        $enPath = $packagePath . '/localizations/en.json';
        $plPath = $packagePath . '/localizations/pl.json';
        $promptPath = storage_path('app/polyglot-prompts/polyglot-relative-clauses-a2.txt');
        ...
        $this->assertFileExists($promptPath);
        $this->assertStringContainsString(
            "require_once __DIR__ . '/PolyglotRelativeClausesLessonSeeder/PolyglotRelativeClausesLessonSeeder.php';",
            File::get($loaderPath)
        );
        ...
        $this->assertSame(
            'Database\\Seeders\\V3\\Polyglot\\PolyglotRelativeClausesLessonSeeder',
            $definition['seeder']['class'] ?? null
        );
        $this->assertSame('polyglot-relative-clauses-a2', $definition['saved_test']['slug'] ?? null);
        $this->assertSame(24, count($definition['questions'] ?? []));
        $this->assertSame(
            'polyglot-relative-clauses-a2-q24',
            $definition['saved_test']['question_uuids'][23] ?? null
        );
        $this->assertSame(
            'Database\\Seeders\\V3\\Localizations\\Uk\\Polyglot\\PolyglotRelativeClausesLessonLocalizationSeeder',
            $ukLocalization['seeder']['class'] ?? null
        );
        $this->assertStringContainsString('relative-clauses', File::get($promptPath));
        $this->assertStringContainsString('polyglot-english-a2', File::get($promptPath));
    }

    public function test_generator_driven_a2_lesson_eleven_seeder_works_and_stays_idempotent(): void
    {
        $this->seed(PolyglotPresentPerfectBasicLessonSeeder::class);
        $this->seed(PolyglotPresentPerfectVsPastSimpleLessonSeeder::class);
        $this->seed(PolyglotFirstConditionalLessonSeeder::class);
        ...
        $lesson = SavedGrammarTest::query()
            ->with('questionLinks')
            ->where('slug', 'polyglot-passive-voice-basics-a2')
            ->firstOrFail();
        $questionUuids = collect(range(1, 24))
            ->map(fn (int $number) => sprintf('polyglot-passive-voice-basics-a2-q%02d', $number))
            ->all();
        $persistentQuestionUuids = $resolver->toPersistentMany($questionUuids);
        $theoryPage = $lesson->filters['prompt_generator']['theory_page'] ?? [];
        ...
        $this->assertSame('polyglot-english-a2', $lesson->filters['course_slug'] ?? null);
        $this->assertSame(11, $lesson->filters['lesson_order'] ?? null);
        $this->assertSame('polyglot-relative-clauses-a2', $lesson->filters['previous_lesson_slug'] ?? null);
        $this->assertSame('polyglot-reported-speech-basics-a2', $lesson->filters['next_lesson_slug'] ?? null);
        $this->assertTrue((bool) ($lesson->filters['supports_duplicate_tokens'] ?? false));
        $this->assertSame(
            'Database\\Seeders\\Page_V3\\PassiveVoice\\Basics\\PassiveVoiceFormationRulesTheorySeeder',
            $theoryPage['page_seeder_class'] ?? null
        );
        $this->assertSame('theory-passive-voice-formation-rules', $theoryPage['slug'] ?? null);
        $this->assertSame('passive-voice', $theoryPage['category_slug_path'] ?? null);
        $this->assertSame(
            1,
            SavedGrammarTest::query()->where('slug', 'polyglot-passive-voice-basics-a2')->count()
        );
    }

    public function test_generator_driven_a2_lesson_eleven_package_exists_in_canonical_v3_structure(): void
    {
        $loaderPath = database_path('seeders/V3/Polyglot/PolyglotPassiveVoiceBasicsLessonSeeder.php');
        $packagePath = database_path('seeders/V3/Polyglot/PolyglotPassiveVoiceBasicsLessonSeeder');
        $definitionPath = $packagePath . '/definition.json';
        $realSeederPath = $packagePath . '/PolyglotPassiveVoiceBasicsLessonSeeder.php';
        $ukPath = $packagePath . '/localizations/uk.json';
        $enPath = $packagePath . '/localizations/en.json';
        $plPath = $packagePath . '/localizations/pl.json';
        $promptPath = storage_path('app/polyglot-prompts/polyglot-passive-voice-basics-a2.txt');
        ...
        $this->assertFileExists($promptPath);
        $this->assertStringContainsString(
            "require_once __DIR__ . '/PolyglotPassiveVoiceBasicsLessonSeeder/PolyglotPassiveVoiceBasicsLessonSeeder.php';",
            File::get($loaderPath)
        );
        ...
        $this->assertSame(
            'Database\\Seeders\\V3\\Polyglot\\PolyglotPassiveVoiceBasicsLessonSeeder',
            $definition['seeder']['class'] ?? null
        );
        $this->assertSame('polyglot-passive-voice-basics-a2', $definition['saved_test']['slug'] ?? null);
        $this->assertSame(24, count($definition['questions'] ?? []));
        $this->assertSame(
            'polyglot-passive-voice-basics-a2-q24',
            $definition['saved_test']['question_uuids'][23] ?? null
        );
        $this->assertSame(
            'Database\\Seeders\\V3\\Localizations\\Uk\\Polyglot\\PolyglotPassiveVoiceBasicsLessonLocalizationSeeder',
            $ukLocalization['seeder']['class'] ?? null
        );
        $this->assertStringContainsString('theory-passive-voice-formation-rules', File::get($promptPath));
        $this->assertStringContainsString('polyglot-english-a2', File::get($promptPath));
    }

    public function test_generator_driven_a2_lesson_twelve_seeder_works_and_stays_idempotent(): void
    {
        $this->seed(PolyglotPresentPerfectBasicLessonSeeder::class);
        $this->seed(PolyglotPresentPerfectVsPastSimpleLessonSeeder::class);
        $this->seed(PolyglotFirstConditionalLessonSeeder::class);
        ...
        $lesson = SavedGrammarTest::query()
            ->with('questionLinks')
            ->where('slug', 'polyglot-reported-speech-basics-a2')
            ->firstOrFail();
        $questionUuids = collect(range(1, 24))
            ->map(fn (int $number) => sprintf('polyglot-reported-speech-basics-a2-q%02d', $number))
            ->all();
        $persistentQuestionUuids = $resolver->toPersistentMany($questionUuids);
        $theoryPage = $lesson->filters['prompt_generator']['theory_page'] ?? [];
        ...
        $this->assertSame('polyglot-english-a2', $lesson->filters['course_slug'] ?? null);
        $this->assertSame(12, $lesson->filters['lesson_order'] ?? null);
        $this->assertSame('polyglot-passive-voice-basics-a2', $lesson->filters['previous_lesson_slug'] ?? null);
        $this->assertSame('polyglot-used-to-a2', $lesson->filters['next_lesson_slug'] ?? null);
        $this->assertTrue((bool) ($lesson->filters['supports_duplicate_tokens'] ?? false));
        $this->assertSame(
            'Database\\Seeders\\Page_V3\\ReportedSpeech\\ReportedSpeechStatementsTheorySeeder',
            $theoryPage['page_seeder_class'] ?? null
        );
        $this->assertSame('reported-statements', $theoryPage['slug'] ?? null);
        $this->assertSame('reported-speech', $theoryPage['category_slug_path'] ?? null);
        $this->assertSame(
            1,
            SavedGrammarTest::query()->where('slug', 'polyglot-reported-speech-basics-a2')->count()
        );
    }

    public function test_generator_driven_a2_lesson_twelve_package_exists_in_canonical_v3_structure(): void
    {
        $loaderPath = database_path('seeders/V3/Polyglot/PolyglotReportedSpeechBasicsLessonSeeder.php');
        $packagePath = database_path('seeders/V3/Polyglot/PolyglotReportedSpeechBasicsLessonSeeder');
        $definitionPath = $packagePath . '/definition.json';
        $realSeederPath = $packagePath . '/PolyglotReportedSpeechBasicsLessonSeeder.php';
        $ukPath = $packagePath . '/localizations/uk.json';
        $enPath = $packagePath . '/localizations/en.json';
        $plPath = $packagePath . '/localizations/pl.json';
        $promptPath = storage_path('app/polyglot-prompts/polyglot-reported-speech-basics-a2.txt');
        ...
        $this->assertFileExists($promptPath);
        $this->assertStringContainsString(
            "require_once __DIR__ . '/PolyglotReportedSpeechBasicsLessonSeeder/PolyglotReportedSpeechBasicsLessonSeeder.php';",
            File::get($loaderPath)
        );
        ...
        $this->assertSame(
            'Database\\Seeders\\V3\\Polyglot\\PolyglotReportedSpeechBasicsLessonSeeder',
            $definition['seeder']['class'] ?? null
        );
        $this->assertSame('polyglot-reported-speech-basics-a2', $definition['saved_test']['slug'] ?? null);
        $this->assertSame(24, count($definition['questions'] ?? []));
        $this->assertSame(
            'polyglot-reported-speech-basics-a2-q24',
            $definition['saved_test']['question_uuids'][23] ?? null
        );
        $this->assertSame(
            'Database\\Seeders\\V3\\Localizations\\Uk\\Polyglot\\PolyglotReportedSpeechBasicsLessonLocalizationSeeder',
            $ukLocalization['seeder']['class'] ?? null
        );
        $this->assertStringContainsString('reported-statements', File::get($promptPath));
        $this->assertStringContainsString('polyglot-english-a2', File::get($promptPath));
    }

    public function test_generator_driven_a2_lesson_thirteen_seeder_works_and_stays_idempotent(): void
    {
        $this->seed(PolyglotPresentPerfectBasicLessonSeeder::class);
        $this->seed(PolyglotPresentPerfectVsPastSimpleLessonSeeder::class);
        $this->seed(PolyglotFirstConditionalLessonSeeder::class);
        ...
        $lesson = SavedGrammarTest::query()
            ->with('questionLinks')
            ->where('slug', 'polyglot-used-to-a2')
            ->firstOrFail();
        $questionUuids = collect(range(1, 24))
            ->map(fn (int $number) => sprintf('polyglot-used-to-a2-q%02d', $number))
            ->all();
        $persistentQuestionUuids = $resolver->toPersistentMany($questionUuids);
        $theoryPage = $lesson->filters['prompt_generator']['theory_page'] ?? [];
        ...
        $this->assertSame('polyglot-english-a2', $lesson->filters['course_slug'] ?? null);
        $this->assertSame(13, $lesson->filters['lesson_order'] ?? null);
        $this->assertSame('polyglot-reported-speech-basics-a2', $lesson->filters['previous_lesson_slug'] ?? null);
        $this->assertSame('polyglot-question-tags-basics-a2', $lesson->filters['next_lesson_slug'] ?? null);
        $this->assertTrue((bool) ($lesson->filters['supports_duplicate_tokens'] ?? false));
        $this->assertSame(
            'Database\\Seeders\\Page_V3\\Tenses\\TensesUsedToWouldTheorySeeder',
            $theoryPage['page_seeder_class'] ?? null
        );
        $this->assertSame('used-to-would', $theoryPage['slug'] ?? null);
        $this->assertSame('tenses', $theoryPage['category_slug_path'] ?? null);
        $this->assertSame(
            1,
            SavedGrammarTest::query()->where('slug', 'polyglot-used-to-a2')->count()
        );
    }

    public function test_generator_driven_a2_lesson_thirteen_package_exists_in_canonical_v3_structure(): void
    {
        $loaderPath = database_path('seeders/V3/Polyglot/PolyglotUsedToLessonSeeder.php');
        $packagePath = database_path('seeders/V3/Polyglot/PolyglotUsedToLessonSeeder');
        $definitionPath = $packagePath . '/definition.json';
        $realSeederPath = $packagePath . '/PolyglotUsedToLessonSeeder.php';
        $ukPath = $packagePath . '/localizations/uk.json';
        $enPath = $packagePath . '/localizations/en.json';
        $plPath = $packagePath . '/localizations/pl.json';
        $promptPath = storage_path('app/polyglot-prompts/polyglot-used-to-a2.txt');
        ...
        $this->assertFileExists($promptPath);
        $this->assertStringContainsString(
            "require_once __DIR__ . '/PolyglotUsedToLessonSeeder/PolyglotUsedToLessonSeeder.php';",
            File::get($loaderPath)
        );
        ...
        $this->assertSame(
            'Database\\Seeders\\V3\\Polyglot\\PolyglotUsedToLessonSeeder',
            $definition['seeder']['class'] ?? null
        );
        $this->assertSame('polyglot-used-to-a2', $definition['saved_test']['slug'] ?? null);
        $this->assertSame(24, count($definition['questions'] ?? []));
        $this->assertSame(
            'polyglot-used-to-a2-q24',
            $definition['saved_test']['question_uuids'][23] ?? null
        );
        $this->assertSame(
            'Database\\Seeders\\V3\\Localizations\\Uk\\Polyglot\\PolyglotUsedToLessonLocalizationSeeder',
            $ukLocalization['seeder']['class'] ?? null
        );
        $this->assertStringContainsString('used-to-would', File::get($promptPath));
        $this->assertStringContainsString('polyglot-english-a2', File::get($promptPath));
    }

    public function test_generator_driven_a2_lesson_fourteen_seeder_works_and_stays_idempotent(): void
    {
        $this->seed(PolyglotPresentPerfectBasicLessonSeeder::class);
        $this->seed(PolyglotPresentPerfectVsPastSimpleLessonSeeder::class);
        $this->seed(PolyglotFirstConditionalLessonSeeder::class);
        ...
        $lesson = SavedGrammarTest::query()
            ->with('questionLinks')
            ->where('slug', 'polyglot-question-tags-basics-a2')
            ->firstOrFail();
        $questionUuids = collect(range(1, 24))
            ->map(fn (int $number) => sprintf('polyglot-question-tags-basics-a2-q%02d', $number))
            ->all();
        $persistentQuestionUuids = $resolver->toPersistentMany($questionUuids);
        $theoryPage = $lesson->filters['prompt_generator']['theory_page'] ?? [];
        ...
        $this->assertSame('polyglot-english-a2', $lesson->filters['course_slug'] ?? null);
        $this->assertSame(14, $lesson->filters['lesson_order'] ?? null);
        $this->assertSame('polyglot-used-to-a2', $lesson->filters['previous_lesson_slug'] ?? null);
        $this->assertSame('polyglot-second-conditional-basics-a2', $lesson->filters['next_lesson_slug'] ?? null);
        $this->assertTrue((bool) ($lesson->filters['supports_duplicate_tokens'] ?? false));
        $this->assertSame(
            'Database\\Seeders\\Page_V3\\QuestionsNegations\\TypesOfQuestions\\TypesOfQuestionsQuestionTagsDisjunctiveQuestionsTheorySeeder',
            $theoryPage['page_seeder_class'] ?? null
        );
        $this->assertSame('question-tags-disjunctive-questions-dont-you-isnt-it', $theoryPage['slug'] ?? null);
        $this->assertSame(
            'pytalni-rechennia-ta-zaperechennia/types-of-questions',
            $theoryPage['category_slug_path'] ?? null
        );
        $this->assertSame(
            1,
            SavedGrammarTest::query()->where('slug', 'polyglot-question-tags-basics-a2')->count()
        );
    }

    public function test_generator_driven_a2_lesson_fourteen_package_exists_in_canonical_v3_structure(): void
    {
        $loaderPath = database_path('seeders/V3/Polyglot/PolyglotQuestionTagsBasicsLessonSeeder.php');
        $packagePath = database_path('seeders/V3/Polyglot/PolyglotQuestionTagsBasicsLessonSeeder');
        $definitionPath = $packagePath . '/definition.json';
        $realSeederPath = $packagePath . '/PolyglotQuestionTagsBasicsLessonSeeder.php';
        $ukPath = $packagePath . '/localizations/uk.json';
        $enPath = $packagePath . '/localizations/en.json';
        $plPath = $packagePath . '/localizations/pl.json';
        $promptPath = storage_path('app/polyglot-prompts/polyglot-question-tags-basics-a2.txt');
        ...
        $this->assertFileExists($promptPath);
        $this->assertStringContainsString(
            "require_once __DIR__ . '/PolyglotQuestionTagsBasicsLessonSeeder/PolyglotQuestionTagsBasicsLessonSeeder.php';",
            File::get($loaderPath)
        );
        ...
        $this->assertSame(
            'Database\\Seeders\\V3\\Polyglot\\PolyglotQuestionTagsBasicsLessonSeeder',
            $definition['seeder']['class'] ?? null
        );
        $this->assertSame('polyglot-question-tags-basics-a2', $definition['saved_test']['slug'] ?? null);
        $this->assertSame(24, count($definition['questions'] ?? []));
        $this->assertSame(
            'polyglot-question-tags-basics-a2-q24',
            $definition['saved_test']['question_uuids'][23] ?? null
        );
        $this->assertSame(
            'Database\\Seeders\\V3\\Localizations\\Uk\\Polyglot\\PolyglotQuestionTagsBasicsLessonLocalizationSeeder',
            $ukLocalization['seeder']['class'] ?? null
        );
        $this->assertStringContainsString(
    ...
    public function test_generator_driven_a2_lesson_fifteen_seeder_works_and_stays_idempotent(): void
    {
        $this->seed(PolyglotPresentPerfectBasicLessonSeeder::class);
        $this->seed(PolyglotPresentPerfectVsPastSimpleLessonSeeder::class);
        $this->seed(PolyglotFirstConditionalLessonSeeder::class);
        ...
        $lesson = SavedGrammarTest::query()
            ->with('questionLinks')
            ->where('slug', 'polyglot-second-conditional-basics-a2')
            ->firstOrFail();
        $questionUuids = collect(range(1, 24))
            ->map(fn (int $number) => sprintf('polyglot-second-conditional-basics-a2-q%02d', $number))
            ->all();
        $persistentQuestionUuids = $resolver->toPersistentMany($questionUuids);
        $theoryPage = $lesson->filters['prompt_generator']['theory_page'] ?? [];
        ...
        $this->assertSame('polyglot-english-a2', $lesson->filters['course_slug'] ?? null);
        $this->assertSame(15, $lesson->filters['lesson_order'] ?? null);
        $this->assertSame('polyglot-question-tags-basics-a2', $lesson->filters['previous_lesson_slug'] ?? null);
        $this->assertSame('polyglot-final-drill-a2', $lesson->filters['next_lesson_slug'] ?? null);
        $this->assertTrue((bool) ($lesson->filters['supports_duplicate_tokens'] ?? false));
        $this->assertSame(
            'Database\\Seeders\\Page_V3\\Conditionals\\ConditionalsSecondTheorySeeder',
            $theoryPage['page_seeder_class'] ?? null
        );
        $this->assertSame('second-conditional', $theoryPage['slug'] ?? null);
        $this->assertSame('conditionals', $theoryPage['category_slug_path'] ?? null);
        $this->assertSame(
            1,
            SavedGrammarTest::query()->where('slug', 'polyglot-second-conditional-basics-a2')->count()
        );
    }

    public function test_generator_driven_a2_lesson_fifteen_package_exists_in_canonical_v3_structure(): void
    {
        $loaderPath = database_path('seeders/V3/Polyglot/PolyglotSecondConditionalBasicsLessonSeeder.php');
        $packagePath = database_path('seeders/V3/Polyglot/PolyglotSecondConditionalBasicsLessonSeeder');
        $definitionPath = $packagePath . '/definition.json';
        $realSeederPath = $packagePath . '/PolyglotSecondConditionalBasicsLessonSeeder.php';
        $ukPath = $packagePath . '/localizations/uk.json';
        $enPath = $packagePath . '/localizations/en.json';
        $plPath = $packagePath . '/localizations/pl.json';
        $promptPath = storage_path('app/polyglot-prompts/polyglot-second-conditional-basics-a2.txt');
        ...
        $this->assertFileExists($promptPath);
        $this->assertStringContainsString(
            "require_once __DIR__ . '/PolyglotSecondConditionalBasicsLessonSeeder/PolyglotSecondConditionalBasicsLessonSeeder.php';",
            File::get($loaderPath)
        );
        ...
        $this->assertSame(
            'Database\\Seeders\\V3\\Polyglot\\PolyglotSecondConditionalBasicsLessonSeeder',
            $definition['seeder']['class'] ?? null
        );
        $this->assertSame('polyglot-second-conditional-basics-a2', $definition['saved_test']['slug'] ?? null);
        $this->assertSame(24, count($definition['questions'] ?? []));
        $this->assertSame(
            'polyglot-second-conditional-basics-a2-q24',
            $definition['saved_test']['question_uuids'][23] ?? null
        );
        $this->assertSame(
            'Database\\Seeders\\V3\\Localizations\\Uk\\Polyglot\\PolyglotSecondConditionalBasicsLessonLocalizationSeeder',
            $ukLocalization['seeder']['class'] ?? null
        );
    ...
    public function test_generator_driven_b1_lessons_five_to_nine_seeders_work_and_stay_idempotent(): void
    {
        $lessons = [
            [
                'seeder' => PolyglotFutureContinuousBasicsLessonSeeder::class,
                'slug' => 'polyglot-future-continuous-basics-b1',
                'order' => 5,
                'previous' => 'polyglot-narrative-tenses-basics-b1',
                'next' => 'polyglot-future-perfect-basics-b1',
                'theory_class' => 'Database\\Seeders\\Page_V3\\FutureForms\\FutureContinuous\\FutureContinuousFormsTheorySeeder',
                'theory_slug' => 'future-continuous-forms',
                'theory_category' => 'maibutni-formy/future-continuous',
                'prompt_fragment' => 'future-continuous-forms',
                'localization_class' => 'Database\\Seeders\\V3\\Localizations\\Uk\\Polyglot\\PolyglotFutureContinuousBasicsLessonLocalizationSeeder',
            ],
            [
                'seeder' => PolyglotFuturePerfectBasicsLessonSeeder::class,
                'slug' => 'polyglot-future-perfect-basics-b1',
                'order' => 6,
                'previous' => 'polyglot-future-continuous-basics-b1',
                'next' => 'polyglot-passive-voice-with-modals-b1',
                'theory_class' => 'Database\\Seeders\\Page_V3\\FutureForms\\FuturePerfect\\FuturePerfectFormsTheorySeeder',
                'theory_slug' => 'future-perfect-forms',
                'theory_category' => 'maibutni-formy/future-perfect',
                'prompt_fragment' => 'future-perfect-forms',
                'localization_class' => 'Database\\Seeders\\V3\\Localizations\\Uk\\Polyglot\\PolyglotFuturePerfectBasicsLessonLocalizationSeeder',
            ],
            [
                'seeder' => PolyglotPassiveVoiceWithModalsLessonSeeder::class,
                'slug' => 'polyglot-passive-voice-with-modals-b1',
                'order' => 7,
                'previous' => 'polyglot-future-perfect-basics-b1',
                'next' => 'polyglot-reported-questions-b1',
                'theory_class' => 'Database\\Seeders\\Page_V3\\PassiveVoice\\Basics\\PassiveVoiceModalVerbsTheorySeeder',
                'theory_slug' => 'theory-passive-voice-modal-verbs',
                'theory_category' => 'passive-voice',
                'prompt_fragment' => 'theory-passive-voice-modal-verbs',
                'localization_class' => 'Database\\Seeders\\V3\\Localizations\\Uk\\Polyglot\\PolyglotPassiveVoiceWithModalsLessonLocalizationSeeder',
            ],
            [
                'seeder' => PolyglotReportedQuestionsLessonSeeder::class,
                'slug' => 'polyglot-reported-questions-b1',
                'order' => 8,
                'previous' => 'polyglot-passive-voice-with-modals-b1',
                'next' => 'polyglot-reported-commands-and-requests-b1',
                'theory_class' => 'Database\\Seeders\\Page_V3\\ReportedSpeech\\ReportedSpeechQuestionsTheorySeeder',
                'theory_slug' => 'reported-questions',
                'theory_category' => 'reported-speech',
                'prompt_fragment' => 'reported-questions',
                'localization_class' => 'Database\\Seeders\\V3\\Localizations\\Uk\\Polyglot\\PolyglotReportedQuestionsLessonLocalizationSeeder',
            ],
            [
                'seeder' => PolyglotReportedCommandsAndRequestsLessonSeeder::class,
                'slug' => 'polyglot-reported-commands-and-requests-b1',
                'order' => 9,
                'previous' => 'polyglot-reported-questions-b1',
                'next' => 'polyglot-non-defining-relative-clauses-b1',
                'theory_class' => 'Database\\Seeders\\Page_V3\\ReportedSpeech\\ReportedSpeechCommandsRequestsTheorySeeder',
                'theory_slug' => 'reported-commands-and-requests',
                'theory_category' => 'reported-speech',
                'prompt_fragment' => 'reported-commands-and-requests',
                'localization_class' => 'Database\\Seeders\\V3\\Localizations\\Uk\\Polyglot\\PolyglotReportedCommandsAndRequestsLessonLocalizationSeeder',
                'question_count' => 48,
            ],
        ];

        foreach ($lessons as $expected) {
            $this->seed($expected['seeder']);
            $this->seed($expected['seeder']);

            $resolver = app(QuestionUuidResolver::class);
            $classBaseName = substr(strrchr($expected['seeder'], '\\'), 1);
            $packagePath = database_path("seeders/V3/Polyglot/{$classBaseName}");
            $loaderPath = database_path("seeders/V3/Polyglot/{$classBaseName}.php");
            $definitionPath = $packagePath . '/definition.json';
            $realSeederPath = "{$packagePath}/{$classBaseName}.php";
            $ukPath = $packagePath . '/localizations/uk.json';
            $enPath = $packagePath . '/localizations/en.json';
            $plPath = $packagePath . '/localizations/pl.json';
            $promptPath = storage_path("app/polyglot-prompts/{$expected['slug']}.txt");
            $questionCount = $expected['question_count'] ?? 24;
            $questionUuids = collect(range(1, $questionCount))
                ->map(fn (int $number) => sprintf('%s-q%02d', $expected['slug'], $number))
                ->all();
            $persistentQuestionUuids = $resolver->toPersistentMany($questionUuids);
            $lesson = SavedGrammarTest::query()
                ->with('questionLinks')
            ...
            $this->assertFileExists($promptPath);
            $this->assertStringContainsString(
                "require_once __DIR__ . '/{$classBaseName}/{$classBaseName}.php';",
                File::get($loaderPath)
            );
            $this->assertSame($expected['seeder'], $definition['seeder']['class'] ?? null);
            $this->assertSame($expected['slug'], $definition['saved_test']['slug'] ?? null);
            $this->assertSame($questionCount, count($definition['questions'] ?? []));
            $this->assertSame($questionUuids, $definition['saved_test']['question_uuids'] ?? []);
            $this->assertSame(
                $expected['localization_class'],
                $ukLocalization['seeder']['class'] ?? null
            );
            $this->assertStringContainsString($expected['prompt_fragment'], File::get($promptPath));
            $this->assertStringContainsString('polyglot-english-b1', File::get($promptPath));
            $this->assertCount($questionCount, $lesson->questionLinks);
            $this->assertSame($persistentQuestionUuids, $lesson->questionLinks->pluck('question_uuid')->all());
            $this->assertSame($questionCount, Question::query()->whereIn('uuid', $persistentQuestionUuids)->count());
            $this->assertTrue(Question::query()->whereIn('uuid', $persistentQuestionUuids)->get()->every(
                fn (Question $question) => (string) $question->type === Question::TYPE_COMPOSE_TOKENS
            ));
            $this->assertTrue(collect($persistentQuestionUuids)->every(
            ...
            $this->assertSame('polyglot-english-b1', $lesson->filters['course_slug'] ?? null);
            $this->assertSame($expected['order'], $lesson->filters['lesson_order'] ?? null);
            $this->assertSame($expected['previous'], $lesson->filters['previous_lesson_slug'] ?? null);
            $this->assertSame($expected['next'], $lesson->filters['next_lesson_slug'] ?? null);
            $this->assertTrue((bool) ($lesson->filters['supports_duplicate_tokens'] ?? false));
            $this->assertSame($expected['theory_class'], $theoryPage['page_seeder_class'] ?? null);
            $this->assertSame($expected['theory_slug'], $theoryPage['slug'] ?? null);
            $this->assertSame($expected['theory_category'], $theoryPage['category_slug_path'] ?? null);
            $this->assertSame(1, SavedGrammarTest::query()->where('slug', $expected['slug'])->count());
        }
    }
