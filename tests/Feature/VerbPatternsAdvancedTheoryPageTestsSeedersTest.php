...
use Database\Seeders\Page_V3\VerbPatterns\AdvancedGerundInfinitivePatternsTheorySeeder;
use Database\Seeders\Page_V3\VerbPatterns\VerbPatternsCategorySeeder;
use Database\Seeders\V3\Polyglot\PolyglotAdvancedGerundInfinitivePatternsAllLevelsLessonSeeder;
use Database\Seeders\V3\Polyglot\PolyglotAdvancedGerundInfinitivePatternsB2LessonSeeder;
use Database\Seeders\V3\TheoryLinks\AdvancedGerundInfinitivePatternsTheoryLinksSeeder;
use Database\Seeders\V3\VerbPatterns\AdvancedGerundInfinitivePatternsAllLevelsV3Seeder;
...
class VerbPatternsAdvancedTheoryPageTestsSeedersTest extends TestCase
{
    ...
    public function test_advanced_gerund_infinitive_page_has_tests_and_theory_links(): void
    {
        $this->seedVerbPatternsAdvancedStack();
        $this->seed(AdvancedGerundInfinitivePatternsTheoryLinksSeeder::class);
        ...
        $this->assertQuestionCountByLevel($case['v3_seeder'], 'V3');
        $this->assertQuestionCountByLevel($case['polyglot_seeder'], 'Polyglot');
        ...
        $this->assertEveryCoveredQuestionHasTheoryLinks($case['covered_seeders'], $case['covered_question_counts']);
        $this->assertDirectSentenceBuilderQuestionsExposeTheoryBlocks($case['direct_slug'], $case['polyglot_seeder']);
        $this->assertMixedResolvedQuestionsExposeTheoryBlocks($mixedTest, $case['mixed_filter_seeders']);
        ...
        $this->assertAuditReportsAdvancedGerundInfinitiveRouteAsOk();
    }

    private function seedVerbPatternsAdvancedStack(): void
    {
        $this->seed([
            VerbPatternsCategorySeeder::class,
            AdvancedGerundInfinitivePatternsTheorySeeder::class,
            AdvancedGerundInfinitivePatternsAllLevelsV3Seeder::class,
            PolyglotAdvancedGerundInfinitivePatternsAllLevelsLessonSeeder::class,
            PolyglotAdvancedGerundInfinitivePatternsB2LessonSeeder::class,
        ]);
    }
    ...
    {
        return [
            'route' => '/theory/verb-patterns/advanced-gerund-infinitive-patterns',
            'page_slug' => 'advanced-gerund-infinitive-patterns',
            'direct_slug' => 'polyglot-advanced-gerund-infinitive-patterns-all-levels',
            'v3_seeder' => AdvancedGerundInfinitivePatternsAllLevelsV3Seeder::class,
            'polyglot_seeder' => PolyglotAdvancedGerundInfinitivePatternsAllLevelsLessonSeeder::class,
            'mixed_filter_seeders' => [AdvancedGerundInfinitivePatternsAllLevelsV3Seeder::class, PolyglotAdvancedGerundInfinitivePatternsAllLevelsLessonSeeder::class],
            'covered_seeders' => [
                AdvancedGerundInfinitivePatternsAllLevelsV3Seeder::class,
                PolyglotAdvancedGerundInfinitivePatternsAllLevelsLessonSeeder::class,
                PolyglotAdvancedGerundInfinitivePatternsB2LessonSeeder::class,
            ],
            'covered_question_counts' => [PolyglotAdvancedGerundInfinitivePatternsB2LessonSeeder::class => 48],
            'links_seeder' => AdvancedGerundInfinitivePatternsTheoryLinksSeeder::class,
        ];
    }
     ...
     */
    private function assertEveryCoveredQuestionHasTheoryLinks(array $seederClasses, array $expectedCounts): void
    {
        $questions = Question::query()->whereIn('seeder', $seederClasses)->get(['id', 'uuid', 'seeder', 'theory_text_block_uuid']);
        foreach ($seederClasses as $seederClass) {
            $this->assertSame($expectedCounts[$seederClass] ?? 72, $questions->where('seeder', $seederClass)->count(), $seederClass);
        }
        ...
        $this->assertSame($questions->count(), $questions->whereNotNull('theory_text_block_uuid')->count(), 'every covered question should set the legacy theory_text_block_uuid.');
    ...
    private function assertDirectSentenceBuilderQuestionsExposeTheoryBlocks(string $directSlug, string $polyglotSeeder): void
    {
        ...
        $this->assertTrue($questions->every(fn (Question $question): bool => (string) $question->seeder === $polyglotSeeder), 'direct Sentence Builder should use only the page-local Polyglot seeder.');
    ...
    private function assertAuditReportsAdvancedGerundInfinitiveRouteAsOk(): void
    {
        $jsonPath = storage_path('framework/testing/verb-patterns-advanced-theory-links-audit.json');
        $markdownPath = storage_path('framework/testing/verb-patterns-advanced-theory-links-audit.md');
        ...
        $row = collect($audit['pages'] ?? [])->firstWhere('route', '/theory/verb-patterns/advanced-gerund-infinitive-patterns');
