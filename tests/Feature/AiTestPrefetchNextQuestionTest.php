...
class AiTestPrefetchNextQuestionTest extends TestCase
{
    ...
    public function next_endpoint_prefetches_question_and_step_uses_it(): void
    {
        ...
        $this->mock(ChatGPTService::class, function ($mock) use ($q1, $q2) {
            $mock->shouldReceive('generateGrammarQuestion')
                ->twice()
                ->andReturn(
                    ['question' => $q1, 'model' => 'gpt-5', 'answers' => ['a1' => 'one'], 'verb_hints' => []],
                    ['question' => $q2, 'model' => 'gpt-5', 'answers' => ['a1' => 'two'], 'verb_hints' => []]
                );
