...
class AutoGeneratedTestServiceTest extends TestCase
{
    ...
    /** @test */
    public function it_generates_saved_grammar_tests_for_level_pairs_with_enough_questions()
    {
        // Create a tag
        ...
        // Check first test is a SavedGrammarTest model
        $firstTest = $tests->first();
        $this->assertInstanceOf(SavedGrammarTest::class, $firstTest);
    ...
    /** @test */
    public function it_reuses_existing_tests_with_same_slug()
    {
        $tag = Tag::factory()->create(['name' => 'test-tag']);
        ...
        $firstTestId1 = $tests1->first()->id;
        
        // Generate tests second time with same context
        $tests2 = $this->service->generateTests(collect([$tag]), 'Same Context');
        $firstTestId2 = $tests2->first()->id;
        
        // Should reuse the same test
        $this->assertEquals($firstTestId1, $firstTestId2);
        
        // Should not create duplicate tests
        $this->assertEquals(1, SavedGrammarTest::count());
    }
}
