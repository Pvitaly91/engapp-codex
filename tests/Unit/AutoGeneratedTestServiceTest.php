...
class AutoGeneratedTestServiceTest extends TestCase
{
    ...
    public function it_generates_virtual_tests_for_level_pairs_with_enough_questions()
    {
        ...
        foreach ($levels as $level) {
            $this->createQuestionsForLevel($tag, $level, 20);
        }
    ...
    public function it_skips_level_pairs_without_enough_questions()
    {
        ...
        $this->createQuestionsForLevel($tag, 'A1', 10);
    ...
    public function it_filters_by_flag_2_only()
    {
        ...
        $this->createQuestionsForLevel($tag, 'A1', 10, 0);
        $this->createQuestionsForLevel($tag, 'A1', 20);
        $this->createQuestionsForLevel($tag, 'A2', 20);
    ...
    public function it_does_not_persist_tests_to_database()
    {
        ...
        $this->assertEquals(0, SavedGrammarTest::count());
    }

    private function createQuestionsForLevel(Tag $tag, string $level, int $count, int $flag = 2): void
    {
        for ($index = 0; $index < $count; $index++) {
            $question = Question::create([
                'uuid' => (string) Str::uuid(),
                'question' => "Generated {$level} question {$index} {a1}.",
                'difficulty' => 1,
                'level' => $level,
                'flag' => $flag,
