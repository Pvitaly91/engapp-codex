...
async function main(label) {
    ...
    const dir = path.resolve(__dirname, '../../storage/app/seo-m8-local');
    ...
    try {
        for (const mobile of [false, true]) for (const expected of accepted.rows.filter(r => r.source_matches)) {
            ...
            await context.route('**/*', async route => {
                ...
                const reason = requestDecision(q.url()) || (q.isNavigationRequest() &&
                    (q.redirectedFrom() || u.origin !== BASE || u.pathname !== expected.path || u.search) ? 'outside-navigation-plan' : null);
            ...
            try {
                ...
                for (const match of expected.source_matches) {
                    ...
                    if (evidence.columns.length || block.sort_order === 9) {
    ...
    report.pass = report.rows.length === 8 && report.rows.every(r => r.pass);
    save();
