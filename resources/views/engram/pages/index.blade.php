        ...
        <section class="space-y-6">
            ...
            @else
                <header class="space-y-2">
                    <h1 class="text-3xl font-semibold tracking-tight">{{ optional($selectedCategory)->title ?? __('public.pages.title') }}</h1>
            ...
            @if($selectedCategory && $selectedCategory->tags->isNotEmpty())
                <div class="rounded-2xl border border-border/80 bg-card shadow-soft p-6" x-data="{ expanded: false }">
                    <div class="flex items-center justify-between mb-4">
                        <h2 class="text-lg font-semibold">{{ __('public.common.category_tags') }}</h2>
                        <button 
                            @click="expanded = !expanded"
                            class="text-sm text-muted-foreground hover:text-foreground transition flex items-center gap-1"
                            :aria-expanded="expanded"
