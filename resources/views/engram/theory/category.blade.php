    ...
    <div class="min-h-screen">
        ...
        <div class="grid grid-cols-1 gap-8 lg:grid-cols-[280px_minmax(0,1fr)] xl:grid-cols-[320px_minmax(0,1fr)]">
            ...
            <div class="min-w-0 space-y-6">
                {{-- Category Description (if has blocks) - Rendered directly via HTML --}}
                @if($categoryDescription['hasBlocks'] ?? false)
                    @php
                        $descriptionBlocks = $categoryDescription['blocks'] ?? collect();
                        $descriptionSubtitleBlock = $categoryDescription['subtitleBlock'] ?? null;
                        ...
                        $contentBlocks = $descriptionBlocks->reject(fn($b) => in_array($b->type, $excludedTypes));
                        $heroBlock = $descriptionBlocks->firstWhere('type', 'hero') ?? $descriptionBlocks->firstWhere('type', 'hero-v2');
                        ...
                        <div class="relative bg-gradient-to-br from-primary/5 via-transparent to-secondary/5 p-6">
                            ...
                            <div class="flex items-start gap-4">
                                ...
                                <div class="flex-1 min-w-0">
                                    <h2 class="text-xl font-bold text-foreground mb-2">{{ $selectedCategory->title }}</h2>
                                    @if($descriptionSubtitleBlock && !empty($descriptionSubtitleBlock->body))
                                        <div class="text-sm text-muted-foreground leading-relaxed prose prose-sm max-w-none">
                                            {!! $descriptionSubtitleBlock->body !!}
                                        </div>
                                    @endif
