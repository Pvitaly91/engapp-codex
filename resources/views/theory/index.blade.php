...
<div class="nd-page">
        ...
        <div class="relative grid gap-10 lg:grid-cols-[1.02fr_0.98fr] lg:items-center">
            <div class="max-w-3xl">
                ...
                <div class="mt-8 flex flex-col gap-4 sm:flex-row">
                    @if($categories->first())
                        <a href="{{ localized_route($routePrefix . '.category', $categories->first()->slug) }}" class="rounded-xl bg-ocean px-6 py-4 text-center text-base font-extrabold text-white shadow-card transition hover:bg-[#245592]">
                            {{ __('public.theory.start_learning') }}
                        </a>
                    @endif
                    <a href="#theory-categories" class="rounded-xl bg-amber px-6 py-4 text-center text-base font-extrabold text-white shadow-card transition hover:bg-[#df8a24]">
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                ...
                <article class="rounded-[12px] border p-6 shadow-card surface-card" style="border-color: var(--line);">
    ...
    <section id="theory-categories" class="nd-section border-b" style="border-color: var(--line);">
        ...
        <div class="mt-8 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            @forelse($categories as $index => $category)
                ...
                <article class="overflow-hidden rounded-[10px] border shadow-card surface-card-strong" style="border-color: var(--line);">
                    ...
                    <div class="space-y-4 p-5">
                            ...
                            <div>
                                ...
                                <div class="mt-3 space-y-2">
                                    @foreach($category->children->take(4) as $child)
                                        <a href="{{ localized_route($routePrefix . '.category', $child->slug) }}" class="flex items-center justify-between rounded-[12px] border px-3 py-3 text-sm transition hover:-translate-y-0.5 surface-card" style="border-color: var(--line); color: var(--text);">
                            ...
                            <div>
                                ...
                                <div class="mt-3 space-y-2">
                                    @foreach($category->pages->take(4) as $page)
                                        <a href="{{ localized_route($routePrefix . '.show', [$category->slug, $page->slug]) }}" class="flex items-start gap-3 rounded-[12px] border px-3 py-3 text-sm transition hover:-translate-y-0.5 surface-card" style="border-color: var(--line); color: var(--muted);">
                        ...
                        <a href="{{ localized_route($routePrefix . '.category', $category->slug) }}" class="inline-flex items-center gap-2 rounded-[12px] bg-ocean px-4 py-3 text-sm font-extrabold uppercase tracking-[0.18em] text-white transition hover:bg-[#245592]">
            ...
            @empty
                <div class="col-span-full rounded-[12px] border border-dashed p-10 text-center shadow-card surface-card-strong" style="border-color: var(--line);">
        ...
        <div class="grid gap-8 xl:grid-cols-[1.08fr_0.92fr]">
                ...
                <div class="mt-8 space-y-4">
                    @foreach([
                        ...
                        <div class="flex gap-4 rounded-[10px] border p-4 surface-card" style="border-color: var(--line);">
                            <span class="inline-flex h-12 w-12 shrink-0 items-center justify-center rounded-[12px] bg-ocean text-sm font-extrabold text-white">
                                {{ $loop->iteration }}
                            </span>
                            <div>
            ...
            <article class="rounded-[12px] border p-6 shadow-card surface-card" style="border-color: var(--line);">
                ...
                <div class="mt-6 grid gap-4">
                    <a href="{{ localized_route('theory.index') }}" class="rounded-[10px] border p-5 surface-card-strong" style="border-color: var(--line);">
                    ...
                    <a href="{{ localized_route('catalog.tests-cards') }}" class="rounded-[10px] border p-5 surface-card-strong" style="border-color: var(--line);">
                    ...
                    <a href="{{ localized_route('home') }}" class="rounded-[10px] border p-5 surface-card-strong" style="border-color: var(--line);">
