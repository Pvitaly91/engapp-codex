    ...
    <section class="relative overflow-hidden rounded-[30px] border p-6 shadow-card surface-card-strong" style="border-color: var(--line);">
        ...
        <div class="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div class="max-w-3xl">
                ...
                <h1 class="mt-3 font-display text-4xl font-extrabold leading-[1.04] sm:text-5xl">{{ $selectedCategory->title }}</h1>
            ...
            <div class="grid gap-3 sm:grid-cols-2">
                <div class="rounded-[22px] border px-5 py-4 surface-card" style="border-color: var(--line);">
                    ...
                    <p class="mt-2 font-display text-[2rem] font-extrabold leading-none">{{ $categoryPages->count() }}</p>
                </div>
                ...
                <section class="rounded-[28px] border p-5 shadow-card surface-card-strong" style="border-color: var(--line);">
                    <div class="flex items-end justify-between gap-3">
                        <div>
                            ...
                            <h2 class="mt-2 font-display text-[1.7rem] font-extrabold leading-none">{{ __('public.common.categories') }}</h2>
                        </div>
        ...
        <div class="min-w-0 space-y-6">
            ...
            <section class="rounded-[30px] border p-6 shadow-card surface-card-strong" style="border-color: var(--line);">
                <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        ...
                        <h2 class="mt-2 font-display text-[2.2rem] font-extrabold leading-none">{{ $selectedCategory->title }}</h2>
                    </div>
                    ...
                    <div class="mt-8 grid gap-5 md:grid-cols-2">
                        @foreach($categoryPages as $page)
                            <a href="{{ localized_route($routePrefix . '.show', [$selectedCategory->slug, $page->slug]) }}" class="rounded-[24px] border p-5 shadow-card transition hover:-translate-y-1 surface-card" style="border-color: var(--line);">
                                ...
                                <h3 class="mt-5 font-display text-[1.6rem] font-extrabold leading-tight">{{ $page->title }}</h3>
                                @if(!empty($page->text))
            ...
            @if(isset($autoGeneratedTests) && $autoGeneratedTests->isNotEmpty())
                <section class="rounded-[30px] border p-6 shadow-card surface-card-strong" style="border-color: var(--line);">
                    ...
                    <h2 class="mt-2 font-display text-[2rem] font-extrabold">{{ __('public.common.tests_on_topic') }}</h2>
            ...
            @if(isset($relatedTests) && $relatedTests->isNotEmpty())
                <section class="rounded-[30px] border p-6 shadow-card surface-card" style="border-color: var(--line);">
                    ...
                    <h2 class="mt-2 font-display text-[2rem] font-extrabold">{{ __('public.common.related_tests') }}</h2>
                    <div class="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
