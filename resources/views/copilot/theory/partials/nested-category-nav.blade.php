{{--
    Copilot – Nested category navigation partial
    ...
    <div class="category-item" x-data="{ expanded: {{ $isExpanded ? 'true' : 'false' }} }">
        <div class="flex items-center gap-1">
            @if($isExpandable)
                <button
                    ...
                    class="flex h-6 w-6 items-center justify-center rounded text-[var(--cp-muted)] hover:text-[var(--cp-fg)] hover:bg-pilot-50/80 dark:hover:bg-pilot-900/30 transition-colors"
                    type="button"
                >
                    <svg
            ...
            <a
                ...
                class="flex-1 min-w-0 flex items-start justify-between gap-2 rounded-lg py-1.5 text-sm transition-all
                    {{ $isActive
                        ? 'bg-pilot-100 dark:bg-pilot-900/50 px-2 text-pilot-700 dark:text-pilot-300 font-semibold'
                        : 'text-[var(--cp-muted)] hover:text-[var(--cp-fg)] hover:bg-pilot-50/80 dark:hover:bg-pilot-900/30' }}"
            >
        ...
        @if($hasOrderedItems)
            <div
                x-show="expanded"
                x-collapse
                class="ml-4 mt-1 space-y-1 border-l border-[var(--cp-border)] pl-2"
            >
