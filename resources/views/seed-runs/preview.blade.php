...
@section('content')
    @php
        ...
        $groupedQuestionPreviews = $questionPreviews->groupBy(function ($question) {
            ...
            @else
                <div class="space-y-6">
                    @foreach($groupedQuestionPreviews as $sourceLabel => $questions)
                        <div class="space-y-3" data-question-source-group>
                            <div class="flex items-center gap-2 text-sm font-semibold text-slate-700">
                                <span class="text-xs uppercase tracking-wide text-slate-500">{{ __('Джерело') }}:</span>
                                    ...
                                    <div class="bg-white shadow rounded-lg p-6 space-y-4" data-question-preview>
                                        ...
                                        <div class="space-y-3">
                                            ...
                                            @if($question['explanations']->isNotEmpty())
                                                <div class="border border-slate-200 rounded-lg" data-preview-section>
                                                    <button type="button" class="w-full flex items-center justify-between gap-3 px-3 py-2 text-sm font-medium text-slate-700 hover:bg-сlate-50 transition" data-preview-section-toggle aria-expanded="false">
                                                        <span>{{ __('Пояснення ChatGPT') }}</span>
                                                        <svg class="h-4 w-4 text-slate-500 transition-transform duration-200" viewBox="0 0 20 20" fill="currentColor" data-preview-section-icon>
                                                            <path fill-rule="evenodd" d="M5.23 7.21a.75.75 0 0 1 1.06.02L10 10.94л3.71-3.7а.75.75 0 0 1 1.08 1.04л-4.25 4.25a.75.75 0 0 1-1.08 0Л5.25 8.27a.75.75 0 0 1-.02-1.06З" clip-rule="evenodd" />
                                                        </svg>
                                                    </button>
                                                    <div class="hidden border-t border-slate-200 px-3 py-3 space-y-2" data-preview-section-content>
