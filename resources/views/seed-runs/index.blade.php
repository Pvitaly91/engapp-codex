...
@section('content')
    @once
        ...
        <style>
            [data-file-editor-wrapper] .CodeMirror {
                height: 24rem;
                min-height: 24rem;
                font-size: 0.875rem;
            ...
            [data-file-editor-wrapper] .CodeMirror-focused {
                outline: 2px solid #6366f1;
                outline-offset: 0;
            }

            [data-file-editor-wrapper] .CodeMirror-gutters {
    ...
    <div id="seeder-file-modal" class="hidden fixed inset-0 z-[60] items-center justify-center" data-load-url="{{ route('seed-runs.file.show') }}">
        ...
        <div class="relative bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 flex flex-col overflow-hidden">
            ...
            <form method="POST" action="{{ route('seed-runs.file.update') }}" class="flex-1 flex flex-col" data-seeder-file-form>
                ...
                <div class="p-6" data-file-editor-wrapper>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const fileModalTextarea = fileModal ? fileModal.querySelector('[data-file-editor]') : null;
            ...
            const ensureFileModalEditorInstance = function () {
                if (fileModalEditorInstance || !fileModalTextarea || typeof window.CodeMirror === 'undefined') {
                    return fileModalEditorInstance;
                }

                fileModalEditorInstance = window.CodeMirror.fromTextArea(fileModalTextarea, {
                ...
                if (fileModalEditorInstance && typeof fileModalEditorInstance.setSize === 'function') {
                    fileModalEditorInstance.setSize('100%', '100%');
                }

                return fileModalEditorInstance;
            };

            const withFileModalEditor = function (callback, { create = false } = {}) {
                if (typeof callback !== 'function') {
                    return;
                }

                let instance = fileModalEditorInstance;

                if (!instance && create) {
                    instance = ensureFileModalEditorInstance();
                }
            ...
            const setFileModalEditorDisabled = function (disabled, { create = false } = {}) {
                if (fileModalTextarea) {
                    if (disabled) {
                        fileModalTextarea.setAttribute('disabled', 'disabled');
                    } else {
                        fileModalTextarea.removeAttribute('disabled');
                    }
                }

                withFileModalEditor(function (editor) {
            ...
            const setFileModalEditorValue = function (value, { create = false, resetHistory = false } = {}) {
                const normalizedValue = typeof value === 'string' ? value : '';

                if (fileModalTextarea) {
                    fileModalTextarea.value = normalizedValue;
                }

                withFileModalEditor(function (editor) {
                    editor.setValue(normalizedValue);

                    if (resetHistory && typeof editor.clearHistory === 'function') {
                        editor.clearHistory();
                    }
            ...
            const getFileModalEditorValue = function () {
                if (fileModalEditorInstance && typeof fileModalEditorInstance.getValue === 'function') {
                    return fileModalEditorInstance.getValue();
                }

                return fileModalTextarea ? (fileModalTextarea.value || '') : '';
            };
            ...
            const focusFileModalEditor = function () {
                if (fileModalEditorInstance) {
                    ...
                    if (typeof fileModalEditorInstance.getDoc === 'function') {
                ...
                if (fileModalTextarea) {
                    const length = fileModalTextarea.value.length;
                    fileModalTextarea.focus({ preventScroll: true });
                    fileModalTextarea.setSelectionRange(length, length);
                }
            };
            ...
            const resetFileModal = function () {
                ...
                setFileModalEditorValue('', { create: false, resetHistory: true });
                setFileModalEditorDisabled(true, { create: false });
            ...
            const loadSeederFile = async function (className, displayName) {
                ...
                try {
                    ...
                    setFileModalEditorDisabled(false, { create: true });
                    setFileModalEditorValue(payload && typeof payload.contents === 'string' ? payload.contents : '', { create: true, resetHistory: true });
            ...
            if (fileModalForm && fileModalTextarea) {
                fileModalForm.addEventListener('submit', async function (event) {
                    ...
                    try {
                        ...
                        if (payload && typeof payload.contents === 'string') {
                            setFileModalEditorValue(payload.contents, { create: false, resetHistory: true });
                        }
