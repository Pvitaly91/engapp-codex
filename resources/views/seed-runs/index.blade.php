...
@section('content')
    ...
    <style>
        ...
        #seeder-file-modal .CodeMirror pre {
            padding-left: 1rem;
        }

        #seeder-file-modal .CodeMirror-activeline-background {
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const initCodeMirrorEditor = function () {
                if (!fileModalEditor || typeof window.CodeMirror === 'undefined') {
                    return null;
                }
                ...
                codeMirrorEditor = window.CodeMirror.fromTextArea(fileModalEditor, {
                ...
                if (wrapper) {
                    wrapper.classList.add('border', 'border-slate-200', 'rounded-lg', 'overflow-hidden');
                }

                return codeMirrorEditor;
            };

            const setEditorDisabledState = function (disabled) {
                if (!fileModalEditor) {
                    return;
                }
                ...
                const editorInstance = initCodeMirrorEditor();
            ...
            const focusEditor = function () {
                ...
                if (editorInstance) {
                    editorInstance.refresh();
                    const totalLines = editorInstance.lineCount();
                    const lastLineIndex = Math.max(totalLines - 1, 0);
                    const lastLineContent = editorInstance.getLine(lastLineIndex) || '';
                    editorInstance.focus();
                    editorInstance.setCursor({ line: lastLineIndex, ch: lastLineContent.length });
            ...
            if (fileModalForm && fileModalEditor) {
                fileModalForm.addEventListener('submit', async function (event) {
                    ...
                    if (editorInstance) {
                        editorInstance.save();
                        contents = editorInstance.getValue();
                    } else if (fileModalEditor) {
                        contents = fileModalEditor.value || '';
                    }
                    ...
                    try {
                        ...
                        if (payload && typeof payload.contents === 'string') {
                            setEditorValue(payload.contents);
                        }
