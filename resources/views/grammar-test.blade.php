...
<div class="max-w-7xl mx-auto p-4 space-y-6">
    @php
        ...
        $preparedSeederNodes = $seederGroups
            ->map(function ($group) use ($selectedSeederClasses, $selectedSourceCollection, $recentSeederClasses, $recentSeederOrdinals) {
                $className = $group['seeder'] ?? null;
                ...
                $segments = collect(explode('\\\\', $displaySeederName))
        ...
        foreach ($preparedSeederNodes as $node) {
            $segments = $node['segments'] ?? [];

            if (empty($segments)) {
                $seederTreeStructure['seeders'][] = $node;
                continue;
            }

            $current =& $seederTreeStructure;

            foreach ($segments as $index => $segment) {
                $isLeaf = $index === count($segments) - 1;

                if ($isLeaf) {
                    $current['seeders'][] = $node;
                } else {
        ...
        $transformSeederTree = null;
        $transformSeederTree = function ($node) use (&$transformSeederTree) {
            $nodes = [];
            $totalSeeders = 0;
            $hasActive = false;
            ...
            foreach ($folders as $folder) {
                $childResult = $transformSeederTree($folder);

                $nodes[] = [
                    'type' => 'folder',
                    'name' => $folder['name'] ?? '',
                    'children' => $childResult['nodes'],
                    'seeder_count' => $childResult['total_seeders'],
                    'has_active_descendant' => $childResult['has_active'],
                ];

                $totalSeeders += $childResult['total_seeders'];

                if ($childResult['has_active']) {
                    $hasActive = true;
                }
            }
            ...
            usort($seeders, function ($a, $b) {
                return strnatcasecmp($a['leaf_name'] ?? '', $b['leaf_name'] ?? '');
            });

            foreach ($seeders as $seeder) {
                $nodes[] = array_merge($seeder, [
                    'type' => 'seeder',
                ]);

                $totalSeeders++;

                if (!empty($seeder['is_active'])) {
                    $hasActive = true;
                }
            }
        ...
        $seederTreeNodes = $transformSeederTree($seederTreeStructure)['nodes'];
    @endphp
