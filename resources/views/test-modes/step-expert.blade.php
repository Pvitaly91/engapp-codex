        ...
        <header class="mb-6 sm:mb-12">
            <div class="text-center space-y-3 sm:space-y-4">
                <div class="inline-flex items-center px-3.5 py-1.5 sm:px-4 sm:py-2 rounded-full bg-gradient-to-r from-indigo-100 to-purple-100 text-indigo-700 text-sm font-semibold mb-3 sm:mb-4">
                    ...
                    {{ __('frontend.tests.templates.step_expert.badge') }}
                </div>
                ...
                <p class="text-[15px] sm:text-lg text-gray-600 max-w-2xl mx-auto">
                    {{ __('frontend.tests.templates.step_expert.description') }}
                </p>
            </div>
        </header>
  ...
  const sentence = renderSentence(q);
    wrap.innerHTML = `
      ...
      <div class="flex gap-2 mb-4">
        <button id="check" class="px-6 py-3.5 sm:px-8 sm:py-4 rounded-2xl bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-semibold shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200 ${q.done ? 'opacity-50 cursor-not-allowed' : ''}" ${q.done ? 'disabled' : ''}>${html(testUi('actions.check_answer'))}</button>
      </div>
