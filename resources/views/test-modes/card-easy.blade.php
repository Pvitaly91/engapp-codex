        ...
        <div class="sticky-test-header sticky top-0 z-30 max-h-[40vh] md:max-h-none" id="sticky-header">
            <div class="sticky-inner space-y-2.5 sm:space-y-4 rounded-2xl border border-indigo-100 bg-white/90 p-2.5 sm:p-5 lg:p-6 shadow backdrop-blur-md transition-all duration-300">
                ...
                <div class="progress-section bg-gradient-to-r from-indigo-50 via-white to-purple-50 rounded-2xl border border-indigo-100 p-2.5 sm:p-4 shadow-inner transition-all duration-300">
                    <div class="progress-layout grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_auto] items-center gap-3 sm:gap-4">
                        <div class="progress-main min-w-0 space-y-2 sm:space-y-3">
                        ...
                        <div class="progress-actions flex justify-end lg:self-stretch">
                            <button
                                type="button"
                                id="restart-test"
  ...
  state.items.forEach((q, idx) => {
    ...
    card.innerHTML = `
      <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3.5 sm:gap-4 mb-5 sm:mb-6">
        <div class="flex-1">
          ...
          ${getTheoryBlocks(q).length ? `<button type="button" class="theory-btn ml-2 sm:ml-3 inline-flex items-center text-[13px] sm:text-sm text-emerald-600 hover:text-emerald-700 font-medium transition-colors" data-theory-idx="${idx}">
            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    card.addEventListener('click', (e) => {
      ...
      if (reorderButton) {
        e.preventDefault();
        e.stopPropagation();
        const result = applySentenceReorderAction(q, reorderButton);
        if (!result) return;
        if (result === 'checked') {
          state.answered += 1;
          if (q.feedback === 'correct') state.correct += 1;
        }
        rerenderCard(idx);
        updateProgress();
        checkAllDone();
...
function getTheoryBlocks(q) {
  // Prefer the multi-block payload; fall back to the legacy single block
  // so older cached sessions keep showing at least one theory hint.
...
function renderTheoryBlockContent(block) {
  // Note: body fields here come from trusted server-side theory seeders and
  // may include intentional HTML formatting (e.g., <strong>). Render them
  // raw — escaping would surface the literal tags to users (see test pages).
  let content = '';
  try {
    ...
    // If body is not valid JSON, it may be raw HTML from trusted seeders.
