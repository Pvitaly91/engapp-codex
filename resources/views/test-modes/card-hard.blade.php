    ...
    <div class="max-w-5xl -mx-3 sm:mx-auto px-0 sm:px-5 md:px-6 lg:px-8 py-6 sm:py-10">
        ...
        <header class="mb-6 sm:mb-12">
            <div class="text-center space-y-3 sm:space-y-4">
                <div class="inline-flex items-center px-3.5 py-1.5 sm:px-4 sm:py-2 rounded-full bg-gradient-to-r from-indigo-100 to-purple-100 text-indigo-700 text-sm font-semibold mb-3 sm:mb-4">
                    ...
                    {{ __('frontend.tests.templates.card_hard.badge') }}
                </div>
                ...
                <p class="text-[15px] sm:text-lg text-gray-600 max-w-2xl mx-auto">
                    {{ __('frontend.tests.templates.card_hard.description') }}
                </p>
            </div>
        </header>
        ...
        <main class="mt-6 sm:mt-8 rounded-3xl bg-white shadow-xl border border-gray-100 p-4 sm:p-6 lg:p-8 space-y-5 sm:space-y-6">
            ...
            <div id="final-check" class="pt-3 sm:pt-4">
                <button id="check-all" type="button" class="w-full sm:w-auto px-8 py-4 sm:px-12 sm:py-5 rounded-2xl bg-gradient-to-r from-indigo-600 to-purple-600 text-[15px] sm:text-lg font-semibold shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200">
                    <span class="flex items-center justify-center">
                        <svg class="w-6 h-6 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                        </svg>
                        {{ __('frontend.tests.actions.check_all') }}
                    </span>
                </button>
            </div>
  ...
  const sentence = renderSentence(q, idx);
    card.innerHTML = `
      ...
      <div class="flex-1">
        ...
        ${getTheoryBlocks(q).length ? `<div class="mt-3">
          <button type="button" class="theory-btn inline-flex items-center text-[13px] sm:text-sm text-emerald-600 hover:text-emerald-700 font-medium transition-colors" data-theory-idx="${idx}">
...
function updateProgress() {
  ...
  document.getElementById('score-label').textContent = testUi('progress.accuracy_value', { value: pct(state.correct, state.items.length) });
