...
@php
    ...
    $contentBlocks        = $blocks->reject(fn($b) => in_array($b->type, ['hero', 'hero-v2', 'navigation-chips']));
...
<div class="space-y-8">
    ...
    <div class="grid grid-cols-1 gap-8 lg:grid-cols-[280px_minmax(0,1fr)] xl:grid-cols-[300px_minmax(0,1fr)]">
        ...
        <aside class="hidden lg:block">
            <div id="nd-theory-sidebar" class="sticky top-24 max-h-[calc(100vh-7rem)] space-y-4 overflow-y-auto pr-1">
                                ...
                                <a
                                    ...
                                    class="block rounded-xl px-3 py-2 text-sm transition-colors
                                        {{ $isCurrentPage
                                            ...
                                            : 'text-steel hover:bg-ocean/6 hover:text-ocean' }}"
                            ...
                            @foreach($tocBlocks as $tocBlock)
                                ...
                                @if(!empty($tocData['title']))
                                    <a
                                        ...
                                        class="flex items-center gap-2 rounded-xl px-3 py-2 text-sm text-steel transition hover:bg-ocean/6 hover:text-ocean"
                                    >
        ...
        <div class="min-w-0 space-y-6 rounded-[1.75rem] border border-line bg-shell p-6 shadow-card">

            {{-- Breadcrumb --}}
            @if(!empty($breadcrumbs))
                <nav class="flex items-center gap-1.5 text-xs text-steel" aria-label="Breadcrumb">
                    @foreach($breadcrumbs as $index => $crumb)
                        @if(!empty($crumb['url']))
                            <a href="{{ $crumb['url'] }}" class="transition hover:text-ocean truncate max-w-[120px]">{{ $crumb['label'] }}</a>
                        @else
                            <span class="font-medium text-night truncate max-w-[180px]">{{ $crumb['label'] }}</span>
                        @endif
            ...
            {{-- ── Hero title card ── --}}
            <header class="relative overflow-hidden rounded-2xl border border-line bg-[radial-gradient(circle_at_top_left,rgba(47,103,177,0.09),transparent_40%),#f5fbff]">
                {{-- Grid decoration --}}
                <div class="pointer-events-none absolute inset-0 opacity-30">
                    <svg class="h-full w-full text-ocean/15" viewBox="0 0 100 100" preserveAspectRatio="none">
                        <defs>
                            <pattern id="nd-page-grid" width="10" height="10" patternUnits="userSpaceOnUse">
                                <path d="M 10 0 L 0 0 0 10" fill="none" stroke="currentColor" stroke-width="0.5"/>
                            </pattern>
                        </defs>
                        <rect width="100%" height="100%" fill="url(#nd-page-grid)"/>
                    </svg>
                </div>
                {{-- Amber accent --}}
                <div class="pointer-events-none absolute -right-4 -top-4 h-24 w-24 rounded-full bg-amber/15 blur-2xl"></div>
                ...
                <div class="relative px-6 py-8 md:px-8 md:py-10">
                    @if(!empty($heroData['level']))
                        <div class="mb-4 flex items-center gap-3">
                            ...
                            <span class="h-1 w-1 rounded-full bg-line"></span>
                            <span class="text-xs text-steel">Теорія</span>
                        </div>
                    @endif
            ...
            @if(!empty($heroData['rules']))
                <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach($heroData['rules'] as $index => $rule)
                        ...
                        <article class="group rounded-xl border border-line {{ $borderColor }} border-l-4 bg-shell p-4 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-card">
                            <div class="flex items-start gap-3">
                                ...
                                <div class="min-w-0 flex-1">
                                    @if(!empty($rule['label']))
                                        <span class="mb-1 block text-xs font-bold uppercase tracking-wider text-steel">
                                            {{ $rule['label'] }}
                                        </span>
                                    @endif
                                    <p class="text-sm leading-relaxed text-night opacity-80">
                                        {!! $rule['text'] ?? '' !!}
                                    </p>
                                    @if(!empty($rule['example']))
                                        <code class="mt-2 block rounded-lg bg-ocean/6 px-3 py-2 font-mono text-xs text-steel">
                                            {{ $rule['example'] }}
                                        </code>
                                    @endif
                                </div>
            ...
            {{-- ── Navigation chips footer ── --}}
            @if($navBlock)
                @php($navData = json_decode($navBlock->body ?? '[]', true) ?? [])
                @if(!empty($navData['items']))
                    <nav class="mt-8 rounded-2xl border border-dashed border-line bg-ocean/4 p-6">
                        @if(!empty($navData['title']))
                            <h3 class="mb-4 text-sm font-bold text-steel">{{ $navData['title'] }}</h3>
                        @endif
                        <div class="flex flex-wrap gap-2">
                            @foreach($navData['items'] as $item)
                                @if(!empty($item['current']))
                                    <span class="inline-flex items-center gap-2 rounded-xl border border-ocean/30 bg-ocean/10 px-4 py-2 text-sm font-medium text-ocean">
                                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                ...
                                @else
                                    <a
                                        href="{{ $item['url'] ?? '#' }}"
                                        class="inline-flex items-center gap-2 rounded-xl border border-line bg-shell px-4 py-2 text-sm font-medium text-steel transition hover:border-ocean hover:text-ocean hover:bg-ocean/6"
                                    >
                                        {{ $item['label'] ?? '' }}
            ...
            @if(isset($autoGeneratedTests) && $autoGeneratedTests->isNotEmpty())
                <section class="rounded-2xl border border-line bg-mist p-6">
                    <div class="mb-5 flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-steel">Перевір свої знання з цієї теми</p>
                        </div>
                    </div>
                                ...
                                <a
                                    ...
                                    class="block rounded-xl px-3 py-2 text-sm
                                        {{ $isCurrentPage
                                            ...
                                            : 'text-steel hover:bg-ocean/8 hover:text-ocean' }}"
                                >
                                    {{ $pageItem->title }}
                                </a>
                            @endforeach
