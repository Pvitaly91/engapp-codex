        ...
        @if($question->tags->count())
            <div class="flex flex-wrap gap-1">
                ...
                @foreach($question->tags as $tag)
                    <a href="{{ localized_route('saved-tests.cards', ['tag' => $tag->name]) }}" class="inline-flex px-2 py-0.5 rounded text-xs font-semibold hover:underline {{ $colors[$loop->index % count($colors)] }}">{{ $tag->name }}</a>
                @endforeach
            </div>
        @endif
