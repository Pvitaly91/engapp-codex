...
function renderMarkerTheoryPanel(idx, marker, block) {
  ...
  const pageLink = block.page_url 
    ? `<a href="${html(block.page_url)}" target="_blank" class="mt-4 inline-flex items-center justify-center gap-2 px-3.5 py-2.5 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 text-sm font-semibold text-white shadow-md hover:shadow-lg transform hover:-translate-y-0.5 transition-all">
        <span>${block.page_title ? html(block.page_title) : 'Open theory page'}</span>
