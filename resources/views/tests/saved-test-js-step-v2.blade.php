...
function fetchMarkerTheory(idx, marker) {
  ...
  if (Object.prototype.hasOwnProperty.call(item.markerTheoryCache, marker)) {
    renderMarkerTheoryPanel(marker, item.markerTheoryCache[marker]);
    return;
  }
  ...
  fetch(MARKER_THEORY_URL, {
    ...
    .then((data) => {
      ...
      renderMarkerTheoryPanel(marker, theoryBlock);
      persistState(state);
    })
    .catch((error) => {
      console.error(error);
      item.markerTheoryCache[marker] = null;
      renderMarkerTheoryPanel(marker, null);
    })
    .finally(() => showLoader(false));
}

function renderMarkerTheoryPanel(marker, block) {
  const panel = document.getElementById('theory-panel');
  if (!panel) return;
